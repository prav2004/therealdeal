...
function getWeeklyTaskState(user, weekKey) {
  ...
  if (weekly.weekKey !== weekKey) {
    return { weekKey, claims: {}, eligibleParlay: false };
  }
  return {
    weekKey,
    claims: weekly.claims && typeof weekly.claims === 'object' ? weekly.claims : {},
    eligibleParlay: !!weekly.eligibleParlay
  };
}
...
async function createUserProfile(uid, email, profileData = {}) {
  ...
  return firestore.runTransaction(async (tx) => {
    ...
    const doc = {
      ...
      firstBetClaimedAt: null,
      firstParlayClaimedAt: null,
      dailyTasks: { dateKey: todayKey, betsPlaced: 0, wins: 0, sports: [], claims: {} },
      weeklyTasks: { weekKey, claims: {}, eligibleParlay: false },
 ...
 * betData: { type: 'single'|'parlay', stake: number, selections: [{eventId, league, marketType, pick, odds}] }
 */
async function placeBet(uid, betData) {
  ...
  for (const s of selections) {
    if (!isFinite(s.odds) || s.odds < 1.01 || s.odds > 20) throw new Error('Invalid odds on selection');
  }
  ...
  return firestore.runTransaction(async (tx) => {
    ...
    // Mark user as eligible for first-bet / first-parlay task reward
    // (tokens are awarded only when the user manually claims on the Tasks page)
    const firstBetRewarded = Boolean(user.firstBetRewarded);
    const firstParlayRewarded = Boolean(user.firstParlayRewarded);
    const nextFirstBetEligible = Boolean(user.firstBetEligible) || (!firstBetRewarded && Number(stats.totalBets || 0) === 0);
    const nextFirstParlayEligible = Boolean(user.firstParlayEligible) || (isParlay && !firstParlayRewarded && Number(stats.totalParlays || 0) === 0);
    ...
    // Apply updates — NO auto-reward tokens; user must claim manually
    tx.set(betRef, betDoc);
    tx.update(userRef, {
      tokenBalance: Math.trunc(newTokenBalance),
      cashBalance: newCashBalance,
      stats: updatedStats,
      xp: nextXp,
      level: nextLevel,
      points: nextPoints,
      firstBetEligible: nextFirstBetEligible,
      firstParlayEligible: nextFirstParlayEligible,
      firstBetRewarded: firstBetRewarded,
      firstParlayRewarded: firstParlayRewarded,
      dailyTasks,
      weeklyTasks
    });
    ...
    return {
      ...
      balances: {
        tokens: Math.trunc(newTokenBalance),
        cash: newCashBalance
      }
    };
  });
...
async function settleBet(betId, result) {
  ...
  return firestore.runTransaction(async (tx) => {
    ...
    if (result.status === 'won') {
      ...
      const legs = Array.isArray(bet.selections) ? bet.selections.length : 0;
      if (bet.type === 'parlay' && legs >= 3) {
        weeklyTasks.eligibleParlay = true;
      }
    }

    // Update individual selection (leg) statuses for parlay display
    let updatedSelections = bet.selections;
    if (Array.isArray(result.selectionOutcomes) && Array.isArray(bet.selections)) {
      updatedSelections = bet.selections.map(sel => {
        const match = result.selectionOutcomes.find(o => o.eventId === sel.eventId);
        if (match) {
          sel.status = match.outcome === 'win' ? 'won' : match.outcome === 'lose' ? 'lost' : 'pending';
        } else {
          // Fallback: use overall bet status (e.g. void applies to all legs)
          sel.status = result.status === 'void' ? 'void' : (sel.status || result.status);
        }
        return sel;
      });
    } else if (Array.isArray(bet.selections)) {
      // No per-leg outcomes provided (e.g. admin settlement) — apply overall status to all legs
      updatedSelections = bet.selections.map(sel => {
        sel.status = sel.status || result.status;
        return sel;
      });
    }
...
async function getUserBets(uid) {
  ...
  // Backfill: if a bet is settled (won/lost/void) but its selections lack
  // individual status fields, infer them so the frontend can display properly.
  bets.forEach(bet => {
    if (!bet || !Array.isArray(bet.selections)) return;
    const overall = String(bet.status || '').toLowerCase();
    if (overall !== 'won' && overall !== 'lost' && overall !== 'void') return;
    bet.selections.forEach(sel => {
      if (!sel.status) {
        // For a won parlay every leg must have won; for a lost parlay at
        // least one leg lost (we don't know which, so mark all as the overall
        // status — this is correct for singles and a reasonable fallback for
        // parlays settled before per-leg tracking was added).
        sel.status = overall;
      }
    });
  });
...
/**
 * Record a player insight view for the current UTC day.
 */
async function recordInsightView(uid) {
  if (!uid) throw new Error('Missing uid');
  const todayKey = getUtcDateKey();
  const userRef = firestore.collection('users').doc(uid);
  return firestore.runTransaction(async (tx) => {
    ...
    if (!snap.exists) throw new Error('User not found');
    const user = snap.data() || {};
    const current = user.dailyInsights || {};
    const count = current.dateKey === todayKey
      ? Number(current.count || 0) + 1
      : 1;
    const next = { dateKey: todayKey, count };
    tx.update(userRef, { dailyInsights: next });
    return next;
  });
}
...
async function claimTaskReward(uid, taskKey) {
  ...
  const dailyWeeklyConfig = {
    'daily-3-picks': { scope: 'daily', reward: 75, rewardType: 'tokens' },
    'daily-2-wins': { scope: 'daily', reward: 100, rewardType: 'tokens' },
    'daily-5-insights': { scope: 'daily', reward: 25, rewardType: 'xp' },
    'daily-2-sports': { scope: 'daily', reward: 80, rewardType: 'tokens' },
    'weekly-5-streak': { scope: 'weekly', reward: 400, rewardType: 'tokens' },
    'weekly-60-winrate': { scope: 'weekly', reward: 250, rewardType: 'tokens' },
    'weekly-3-leg-parlay': { scope: 'weekly', reward: 400, rewardType: 'tokens' },
    'weekly-level-up': { scope: 'weekly', reward: 500, rewardType: 'tokens' }
  };
  const dailyWeekly = dailyWeeklyConfig[task];

  let has3LegParlayWin = false;
  if (dailyWeekly && task === 'weekly-3-leg-parlay') {
    const parlaySnap = await firestore.collection('bets')
      ...
      .limit(20)
      .get();
    parlaySnap.forEach((doc) => {
      const bet = doc.data() || {};
      const legs = Array.isArray(bet.selections) ? bet.selections.length : 0;
      if (legs >= 3) has3LegParlayWin = true;
    });
  }
  ...
  if (dailyWeekly) {
    ...
    return firestore.runTransaction(async (tx) => {
      ...
      const dailyTasks = getDailyTaskState(user, todayKey);
      const weeklyTasks = getWeeklyTaskState(user, weekKey);
      const dailyInsights = user.dailyInsights || {};
      const insightCount = dailyInsights.dateKey === todayKey ? Number(dailyInsights.count || 0) : 0;

      const claims = dailyWeekly.scope === 'daily' ? dailyTasks.claims : weeklyTasks.claims;
      const existingClaim = claims[task];
      const alreadyClaimed = !!existingClaim;
      const claimAt = typeof existingClaim === 'string' ? existingClaim : null;

      let eligible = false;
      if (task === 'daily-3-picks') eligible = Number(dailyTasks.betsPlaced || 0) >= 3;
      if (task === 'daily-2-wins') eligible = Number(dailyTasks.wins || 0) >= 2;
      if (task === 'daily-5-insights') eligible = insightCount >= 5;
      if (task === 'daily-2-sports') eligible = (dailyTasks.sports || []).length >= 2;
      if (task === 'weekly-5-streak') eligible = Number(user.streakWins || 0) >= 5;
      if (task === 'weekly-60-winrate') {
        const wins = Number(stats.wins || 0);
        const total = Number(stats.totalBets || 0);
        eligible = total >= 10 && (wins / total) >= 0.6;
      }
      if (task === 'weekly-3-leg-parlay') eligible = has3LegParlayWin;
      if (task === 'weekly-level-up') eligible = String(user.level || getLevelFromXp(user.xp || 0)) !== 'Bronze';
      ...
      let nextXp = Number(user.xp || 0);
      let nextLevel = user.level || getLevelFromXp(nextXp);
      let nextPoints = Number(user.points || 0);

      if (alreadyClaimed) {
        if (!claimAt) {
          const nextClaims = Object.assign({}, claims, { [task]: nowIso });
          if (dailyWeekly.scope === 'daily') dailyTasks.claims = nextClaims;
          else weeklyTasks.claims = nextClaims;
          tx.update(userRef, { dailyTasks, weeklyTasks });
        }
        return {
          awarded: false,
          reason: 'already-claimed',
          reward: 0,
          rewardType: dailyWeekly.rewardType,
          claimAt: claimAt || nowIso,
      ...
      if (dailyWeekly.rewardType === 'tokens') {
        nextTokens = Math.trunc(currentTokens + dailyWeekly.reward);
      } else if (dailyWeekly.rewardType === 'xp') {
        nextXp = Number(user.xp || 0) + Number(dailyWeekly.reward || 0);
        nextLevel = getLevelFromXp(nextXp);
        nextPoints = computePoints({ xp: nextXp, stats, streakWins: Number(user.streakWins || 0) });
      }
  ...
  return firestore.runTransaction(async (tx) => {
    ...
    const existingClaimAt = user[config.claimKey] || null;
    ...
    if (alreadyClaimed) {
      if (!existingClaimAt) {
        const nowIso = new Date().toISOString();
        tx.update(userRef, { [config.claimKey]: nowIso });
      }
      return {
        awarded: false,
        reason: 'already-claimed',
        reward: 0,
        rewardType: 'tokens',
        claimAt: existingClaimAt || new Date().toISOString(),
        balances: { tokens: currentTokens, cash: currentCash },
        stats,
        firstBetRewarded: !!user.firstBetRewarded,
        firstParlayRewarded: !!user.firstParlayRewarded,
        firstBetClaimedAt: user.firstBetClaimedAt || null,
        firstParlayClaimedAt: user.firstParlayClaimedAt || null
      };
    }
    if (!eligible) {
      return {
    ...
    tx.update(userRef, {
      tokenBalance: nextTokens,
      [config.flag]: true,
      [config.claimKey]: new Date().toISOString()
    });
    ...
    return {
      ...
      firstBetClaimedAt: task === 'first-bet' ? new Date().toISOString() : (user.firstBetClaimedAt || null),
      firstParlayClaimedAt: task === 'first-parlay' ? new Date().toISOString() : (user.firstParlayClaimedAt || null)
    };
  });
}
