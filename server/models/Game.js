...
class Game {
  ...
  toJSON() {
    return {
      ...
      homeRecord: this.meta.homeRecord || '',   // home-specific record (e.g. "20-5")
      awayRecord: this.meta.awayRecord || '',   // away-specific record (e.g. "12-14")
      recentForm: {
        home: (this.meta.homeForm || []).slice(0, 10),
        away: (this.meta.awayForm || []).slice(0, 10)
      },
      espnOdds: {
        spread: this.meta.espnSpread || '',
        overUnder: this.meta.espnOverUnder || null,
        homeML: this.meta.espnHomeML || null,
        awayML: this.meta.espnAwayML || null
      },
      espnWinProb: {
        home: this.meta.espnHomeWinPct || null,
        away: this.meta.espnAwayWinPct || null
      },
      h2h: this.meta.h2h || { wins: 0, losses: 0, draws: 0, games: [] },
      teamStats: {
        home: this.meta.homeStats || null,
        away: this.meta.awayStats || null
      },
      venue: this.meta.venue || '',
      score: this.score,
      odds: {
  ...
  /**
   * Create from The Odds API event object + extracted odds.
   * @param {string} sport  Pickr sport key: 'nba', 'nfl', 'nhl', 'mlb', 'soccer'
   * @param {object} event  Raw event from The Odds API /v4/sports/{key}/odds
   * @param {object} opts   { oddsData, league }
   */
  static fromOddsApi(sport, event, { oddsData, league } = {}) {
    return new Game({
      sport: String(sport || '').toUpperCase(),
      league: league || String(sport || '').toUpperCase(),
      game_id: event.id || '',
      ...
      start_time: event.commence_time || null,
      status: 'scheduled',
