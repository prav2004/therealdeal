 ...
 * - opts: { user } optional user object to credit balances. If provided, user.tokenBalance will be updated and user.bets updated.
...
function settleBet(bet, settlementResults, opts = {}) {
  ...
  if (bet.type === 'single') {
    ...
    if (leg.outcome === 'win') {
      ...
      // credit payout to cash balance
      if (opts.user) {
        const currentCash = Number(opts.user.cashBalance || opts.user.cash || 0);
        opts.user.cashBalance = Math.round((currentCash + roundDecimals(updatedBet.potentialPayout)) * 100) / 100;
      }
    ...
    if (anyLost) {
      ...
      if (voidLegs.length > 0 && activeLegs.length === 0) {
        ...
        // all legs won
        updatedBet.status = 'won';
        if (opts.user) opts.user.cashBalance = Math.round((Number(opts.user.cashBalance || 0) + roundDecimals(updatedBet.potentialPayout)) * 100) / 100;
        ...
        if (remainingSelections.length === 0) {
          ...
          updatedBet.status = 'won';
          if (opts.user) opts.user.cashBalance = Math.round((Number(opts.user.cashBalance || 0) + payout) * 100) / 100;
        } else {
