...
function buildOddsMap(oddsList) {
  ...
  (oddsList || []).forEach((game) => {
    ...
    if (!gameId) return;
    let homeMoneyline = readStat(game, ['HomeMoneyLine', 'HomeTeamMoneyLine', 'HomeMoneyline']);
    let awayMoneyline = readStat(game, ['AwayMoneyLine', 'AwayTeamMoneyLine', 'AwayMoneyline']);
    let drawMoneyline = readStat(game, ['DrawMoneyLine', 'TieMoneyLine', 'DrawMoneyline']);
    let pointSpread = readStat(game, ['PointSpread', 'PointSpreadValue']);
    let homeSpread = readStat(game, ['HomePointSpread', 'HomeSpread']);
    let awaySpread = readStat(game, ['AwayPointSpread', 'AwaySpread']);
    let homeSpreadPayout = readStat(game, ['HomePointSpreadPayout', 'HomeSpreadPayout']);
    let awaySpreadPayout = readStat(game, ['AwayPointSpreadPayout', 'AwaySpreadPayout']);
    let totalPoints = readStat(game, ['OverUnder', 'Total', 'TotalPoints', 'OverUnderPoints', 'OverUnderTotal', 'TotalScore']);
    let overPayout = readStat(game, ['OverPayout', 'OverMoneyLine', 'OverOdds', 'OverPrice']);
    let underPayout = readStat(game, ['UnderPayout', 'UnderMoneyLine', 'UnderOdds', 'UnderPrice']);
    ...
    nestedKeys.forEach((key) => {
      ...
      entries.forEach((entry) => {
        if (typeof homeMoneyline === 'undefined' || homeMoneyline === null) {
          const v = readStat(entry, ['HomeMoneyLine', 'HomeTeamMoneyLine', 'HomeMoneyline']);
          if (typeof v !== 'undefined' && v !== null) homeMoneyline = v;
        }
        if (typeof awayMoneyline === 'undefined' || awayMoneyline === null) {
          const v = readStat(entry, ['AwayMoneyLine', 'AwayTeamMoneyLine', 'AwayMoneyline']);
          if (typeof v !== 'undefined' && v !== null) awayMoneyline = v;
        }
        if (typeof drawMoneyline === 'undefined' || drawMoneyline === null) {
          const v = readStat(entry, ['DrawMoneyLine', 'TieMoneyLine', 'DrawMoneyline']);
          if (typeof v !== 'undefined' && v !== null) drawMoneyline = v;
        }
        if (typeof pointSpread === 'undefined' || pointSpread === null) {
          const v = readStat(entry, ['PointSpread', 'PointSpreadValue']);
          if (typeof v !== 'undefined' && v !== null) pointSpread = v;
        }
        if (typeof homeSpread === 'undefined' || homeSpread === null) {
          const v = readStat(entry, ['HomePointSpread', 'HomeSpread']);
          if (typeof v !== 'undefined' && v !== null) homeSpread = v;
        }
        if (typeof awaySpread === 'undefined' || awaySpread === null) {
          const v = readStat(entry, ['AwayPointSpread', 'AwaySpread']);
          if (typeof v !== 'undefined' && v !== null) awaySpread = v;
        }
        if (typeof homeSpreadPayout === 'undefined' || homeSpreadPayout === null) {
          const v = readStat(entry, ['HomePointSpreadPayout', 'HomeSpreadPayout']);
          if (typeof v !== 'undefined' && v !== null) homeSpreadPayout = v;
        }
        if (typeof awaySpreadPayout === 'undefined' || awaySpreadPayout === null) {
          const v = readStat(entry, ['AwayPointSpreadPayout', 'AwaySpreadPayout']);
          if (typeof v !== 'undefined' && v !== null) awaySpreadPayout = v;
        }
        if (typeof totalPoints === 'undefined' || totalPoints === null) {
          const v = readStat(entry, ['OverUnder', 'Total', 'TotalPoints', 'OverUnderPoints', 'OverUnderTotal', 'TotalScore']);
          if (typeof v !== 'undefined' && v !== null) totalPoints = v;
        }
        if (typeof overPayout === 'undefined' || overPayout === null) {
          const v = readStat(entry, ['OverPayout', 'OverMoneyLine', 'OverOdds', 'OverPrice']);
          if (typeof v !== 'undefined' && v !== null) overPayout = v;
        }
        if (typeof underPayout === 'undefined' || underPayout === null) {
          const v = readStat(entry, ['UnderPayout', 'UnderMoneyLine', 'UnderOdds', 'UnderPrice']);
          if (typeof v !== 'undefined' && v !== null) underPayout = v;
        }
      });
    });
    map.set(String(gameId), {
      ...
      totals: {
        points: typeof totalPoints !== 'undefined' ? totalPoints : null,
        over: typeof overPayout !== 'undefined' ? overPayout : null,
        under: typeof underPayout !== 'undefined' ? underPayout : null
      }
    });
  });
  return map;
