...
const WITHDRAW_EMAIL_TO = process.env.WITHDRAW_EMAIL_TO || 'pickrapphelp@gmail.com';
const SUPPORT_EMAIL_TO = process.env.SUPPORT_EMAIL_TO || 'pickrapphelp@gmail.com';
const CAREERS_EMAIL_TO = process.env.CAREERS_EMAIL_TO || process.env.WITHDRAW_EMAIL_TO || 'pickrapphelp@gmail.com';
...
function getDateKeyForZone(date, timeZone) {
  ...
  const acc = parts.reduce((out, part) => {
    if (part.type !== 'literal') out[part.type] = part.value;
    return out;
  }, {});
  return `${acc.year}-${acc.month}-${acc.day}`;
}
...
const SPORTSDATAIO_KEY = process.env.SPORTSDATAIO_KEY || SPORTS_DATA_IO_KEY || '';
...
const SPORTSDATA_CACHE_TTL = {
  // Reduce API usage by caching games/odds for one hour.
  odds: 60 * 60 * 1000,
  scores: 60 * 60 * 1000,
  teams: 12 * 60 * 60 * 1000,
...
function buildOddsMap(oddsList) {
  ...
  (oddsList || []).forEach((game) => {
    ...
    const chooseLatest = (current, value, updateMs) => {
      ...
      if (!current || updateMs >= current.updatedMs) {
        return { value, updatedMs: updateMs };
      }
      return current;
    };
    const baseUpdateMs = getUpdateMs(game);
    let homeMoneyline = chooseLatest(null, readStat(game, ['HomeMoneyLine', 'HomeTeamMoneyLine', 'HomeMoneyline']), baseUpdateMs);
    let awayMoneyline = chooseLatest(null, readStat(game, ['AwayMoneyLine', 'AwayTeamMoneyLine', 'AwayMoneyline']), baseUpdateMs);
    let drawMoneyline = chooseLatest(null, readStat(game, ['DrawMoneyLine', 'TieMoneyLine', 'DrawMoneyline']), baseUpdateMs);
    let pointSpread = chooseLatest(null, readStat(game, ['PointSpread', 'PointSpreadValue']), baseUpdateMs);
    let homeSpread = chooseLatest(null, readStat(game, ['HomePointSpread', 'HomeSpread']), baseUpdateMs);
    let awaySpread = chooseLatest(null, readStat(game, ['AwayPointSpread', 'AwaySpread']), baseUpdateMs);
    let homeSpreadPayout = chooseLatest(null, readStat(game, ['HomePointSpreadPayout', 'HomeSpreadPayout']), baseUpdateMs);
    let awaySpreadPayout = chooseLatest(null, readStat(game, ['AwayPointSpreadPayout', 'AwaySpreadPayout']), baseUpdateMs);
    let totalPoints = chooseLatest(null, readStat(game, ['OverUnder', 'Total', 'TotalPoints', 'OverUnderPoints', 'OverUnderTotal', 'TotalScore']), baseUpdateMs);
    let overPayout = chooseLatest(null, readStat(game, ['OverPayout', 'OverMoneyLine', 'OverOdds', 'OverPrice']), baseUpdateMs);
    let underPayout = chooseLatest(null, readStat(game, ['UnderPayout', 'UnderMoneyLine', 'UnderOdds', 'UnderPrice']), baseUpdateMs);

    const nestedKeys = ['PregameOdds', 'Odds', 'GameOdds', 'Books', 'Bookmakers', 'BookmakerOdds'];
    nestedKeys.forEach((key) => {
      const entries = Array.isArray(game && game[key]) ? game[key] : [];
      entries.forEach((entry) => {
        const entryUpdateMs = getUpdateMs(entry) || baseUpdateMs;
        homeMoneyline = chooseLatest(homeMoneyline, readStat(entry, ['HomeMoneyLine', 'HomeTeamMoneyLine', 'HomeMoneyline']), entryUpdateMs);
        awayMoneyline = chooseLatest(awayMoneyline, readStat(entry, ['AwayMoneyLine', 'AwayTeamMoneyLine', 'AwayMoneyline']), entryUpdateMs);
        drawMoneyline = chooseLatest(drawMoneyline, readStat(entry, ['DrawMoneyLine', 'TieMoneyLine', 'DrawMoneyline']), entryUpdateMs);
    ...
    map.set(String(gameId), {
      moneyline: {
        home: homeMoneyline ? homeMoneyline.value : null,
        draw: drawMoneyline ? drawMoneyline.value : null,
        away: awayMoneyline ? awayMoneyline.value : null
      },
      spread: {
...
async function fetchApiSportsGamesByDate(dateKey, season, ttlMs = SPORTSDATA_CACHE_TTL.scores) {
  if (!APISPORTS_KEY) return [];
  const cacheKey = `apisports:nba:games:${season}:${dateKey}`;
  const cached = getSportsDataCache(cacheKey);
  if (cached) return cached;
  const resp = await axios.get(`${APISPORTS_BASE}/games`, {
    headers: getApiSportsHeaders(),
    params: { league: '12', season, date: dateKey },
    timeout: 12000
  });
  const list = resp && resp.data && Array.isArray(resp.data.response) ? resp.data.response : [];
  setSportsDataCache(cacheKey, list, ttlMs);
  return list;
}

function getApiSportsUpdateMs(obj) {
  if (!obj) return 0;
  const raw = readStat(obj, ['update', 'last_update', 'lastUpdate', 'updated_at', 'updatedAt', 'timestamp', 'date']);
  if (!raw) return 0;
  if (typeof raw === 'number') return raw > 1e12 ? raw : raw * 1000;
  const parsed = Date.parse(String(raw));
  return Number.isFinite(parsed) ? parsed : 0;
...
function pickApiSportsMoneyline(response, homeName, awayName) {
  ...
  list.forEach((entry) => {
    const entryUpdated = getApiSportsUpdateMs(entry);
    const bookmakers = entry && entry.bookmakers;
    if (!Array.isArray(bookmakers)) return;
    bookmakers.forEach((bookmaker) => {
      const bookName = String(bookmaker && bookmaker.name || '').toLowerCase();
      const isFanDuel = bookName.includes('fanduel');
      const isDraftKings = bookName.includes('draftkings') || bookName.includes('draft kings');
      const bookUpdated = Math.max(entryUpdated, getApiSportsUpdateMs(bookmaker));
      const bets = bookmaker && bookmaker.bets;
      if (!Array.isArray(bets)) return;
      bets.forEach((bet) => {
        const betUpdated = Math.max(bookUpdated, getApiSportsUpdateMs(bet));
        const name = String(bet && bet.name || '').toLowerCase();
        if (!name || (!name.includes('winner') && !name.includes('moneyline') && !name.includes('home/away') && !name.includes('1x2') && !name.includes('3way') && !name.includes('result'))) return;
        const values = Array.isArray(bet.values) ? bet.values : [];
        values.forEach((value) => {
          const label = String(value && (value.value || value.name) || '').toLowerCase();
          const odd = Number(value && (value.odd || value.price || value.value));
          if (!Number.isFinite(odd) || odd <= 1) return;
          const valueUpdated = Math.max(betUpdated, getApiSportsUpdateMs(value));
          if (label.includes('draw') || label === 'x') {
            // keep latest odds for most recent display
            if (isFanDuel) {
              if (!drawFd || valueUpdated >= drawFdUpdated) {
                drawFd = odd;
                drawFdUpdated = valueUpdated;
              }
            } else if (isDraftKings) {
              if (!drawDk || valueUpdated >= drawDkUpdated) {
                drawDk = odd;
                drawDkUpdated = valueUpdated;
              }
            } else if (!draw || valueUpdated >= drawUpdated) {
              draw = odd;
              drawUpdated = valueUpdated;
            }
            return;
          }
          if (label.includes('home') || label === '1' || (homeKey && label.includes(homeKey))) {
            // keep latest odds for most recent display
            if (isFanDuel) {
              if (!homeFd || valueUpdated >= homeFdUpdated) {
                homeFd = odd;
                homeFdUpdated = valueUpdated;
              }
            } else if (isDraftKings) {
              if (!homeDk || valueUpdated >= homeDkUpdated) {
                homeDk = odd;
                homeDkUpdated = valueUpdated;
              }
            } else if (!home || valueUpdated >= homeUpdated) {
              home = odd;
              homeUpdated = valueUpdated;
            }
            return;
          }
          if (label.includes('away') || label === '2' || (awayKey && label.includes(awayKey))) {
            // keep latest odds for most recent display
            if (isFanDuel) {
              if (!awayFd || valueUpdated >= awayFdUpdated) {
                awayFd = odd;
                awayFdUpdated = valueUpdated;
              }
            } else if (isDraftKings) {
              if (!awayDk || valueUpdated >= awayDkUpdated) {
                awayDk = odd;
                awayDkUpdated = valueUpdated;
              }
            } else if (!away || valueUpdated >= awayUpdated) {
              away = odd;
              awayUpdated = valueUpdated;
            }
          }
        });
      });
  ...
  return {
    home: homeFd || homeDk || home,
    away: awayFd || awayDk || away,
    draw: drawFd || drawDk || draw
  };
}

function parseApiSportsSpread(response) {
  const list = Array.isArray(response) ? response : [];
  const lines = new Map();
  list.forEach((entry) => {
    const bookmakers = entry && entry.bookmakers;
    ...
    bookmakers.forEach((bookmaker) => {
      ...
      bets.forEach((bet) => {
        const name = String(bet && bet.name || '').toLowerCase();
        if (!name || !(name.includes('handicap') || name.includes('spread'))) return;
        if (name.includes('1st') || name.includes('2nd') || name.includes('quarter') || name.includes('half')) return;
        const values = Array.isArray(bet.values) ? bet.values : [];
        values.forEach((value) => {
          const label = String(value && (value.value || value.name) || '');
          const match = label.match(/^(Home|Away)\s*([+-]?\d+(?:\.\d+)?)/i);
          const odd = Number(value && (value.odd || value.price));
          if (!match || !Number.isFinite(odd) || odd <= 1) return;
          const side = match[1].toLowerCase();
          const line = Number(match[2]);
          if (!Number.isFinite(line)) return;
          const key = String(Math.abs(line));
          const existing = lines.get(key) || { line: Math.abs(line), home: null, away: null, homeLine: null, awayLine: null };
          if (side === 'home') {
            if (!existing.home || odd > existing.home) {
              existing.home = odd;
              existing.homeLine = line;
            }
          } else {
            if (!existing.away || odd > existing.away) {
              existing.away = odd;
              existing.awayLine = line;
            }
          }
  ...
  const candidates = Array.from(lines.values())
    .filter((entry) => Number.isFinite(entry.line) && entry.home && entry.away)
    .sort((a, b) => a.line - b.line);
  if (!candidates.length) return { points: null, home: null, away: null, homePayout: null, awayPayout: null };
  const best = candidates[0];
  const homeSpread = Number.isFinite(best.homeLine) ? best.homeLine : (Number.isFinite(best.awayLine) ? -best.awayLine : null);
  const awaySpread = Number.isFinite(best.awayLine) ? best.awayLine : (Number.isFinite(homeSpread) ? -homeSpread : null);
  return {
    points: best.line,
    home: homeSpread,
    away: awaySpread,
    homePayout: best.home,
    awayPayout: best.away
  };
}

function parseApiSportsTotals(response) {
  const list = Array.isArray(response) ? response : [];
  ...
  list.forEach((entry) => {
    ...
    bookmakers.forEach((bookmaker) => {
      ...
      bets.forEach((bet) => {
        const name = String(bet && bet.name || '').toLowerCase();
        if (!name || !name.includes('over/under')) return;
        if (name.includes('1st') || name.includes('2nd') || name.includes('quarter') || name.includes('half')) return;
        const values = Array.isArray(bet.values) ? bet.values : [];
        values.forEach((value) => {
          const label = String(value && (value.value || value.name) || '');
          ...
          const existing = totals.get(key) || { line, over: null, under: null };
          if (side === 'over') {
            if (!existing.over || odd > existing.over) existing.over = odd;
          } else {
            if (!existing.under || odd > existing.under) existing.under = odd;
          }
  ...
  const candidates = Array.from(totals.values())
    .filter((entry) => entry.over && entry.under)
    .sort((a, b) => Math.abs(a.over - a.under) - Math.abs(b.over - b.under));
  if (!candidates.length) return { points: null, over: null, under: null };
  const best = candidates[0];
  return {
    points: best.line,
    over: best.over,
...
async function fetchApiSportsOddsByGame(gameId, season, homeName, awayName, ttlMs = SPORTSDATA_CACHE_TTL.odds) {
  if (!APISPORTS_KEY || !gameId) return { moneyline: { home: null, away: null, draw: null } };
  const cacheKey = `apisports:nba:odds:${season}:${gameId}`;
  const cached = getSportsDataCache(cacheKey);
  if (cached) return cached;
  const resp = await axios.get(`${APISPORTS_BASE}/odds`, {
    headers: getApiSportsHeaders(),
    params: { league: '12', season, game: gameId },
    timeout: 12000
  });
  const list = resp && resp.data && Array.isArray(resp.data.response) ? resp.data.response : [];
  const moneyline = pickApiSportsMoneyline(list, homeName, awayName);
  const spread = parseApiSportsSpread(list);
  const totals = parseApiSportsTotals(list);
  const payload = { moneyline, spread, totals };
  setSportsDataCache(cacheKey, payload, ttlMs);
  return payload;
}

async function buildNbaNextGamesFromApiSports() {
  if (!APISPORTS_KEY) return [];
  const season = getApiSportsNbaSeason();
  const now = new Date();
  const timeZone = process.env.NBA_TIMEZONE || 'America/Toronto';
  const todayKeyLocal = getDateKeyForZone(now, timeZone);
  const tomorrowKeyLocal = getDateKeyForZone(new Date(now.getTime() + 24 * 60 * 60 * 1000), timeZone);

  const getApiSportsStartMs = (game) => {
    const startRaw = game && game.date && game.date.start
      ? game.date.start
      : (game && game.date ? game.date : (game && game.timestamp ? game.timestamp : null));
    if (!startRaw) return null;
    if (typeof startRaw === 'number') return startRaw > 1e12 ? startRaw : startRaw * 1000;
    const parsed = Date.parse(String(startRaw));
    return Number.isFinite(parsed) ? parsed : null;
  };

  const gamesById = new Map();
  const dateKeys = [todayKeyLocal, tomorrowKeyLocal];
  for (const dateKey of dateKeys) {
    const list = await fetchApiSportsGamesByDate(dateKey, season, SPORTSDATA_CACHE_TTL.scores);
    list.forEach((game) => {
      const id = game && game.id ? String(game.id) : null;
      if (!id) return;
  ...
  let nextGamesDate = null;
  if (!games.length) {
    const lookaheadDays = 14;
    for (let offset = 0; offset <= lookaheadDays; offset += 1) {
      const dateKey = getDateKeyForZone(new Date(now.getTime() + (offset * 24 * 60 * 60 * 1000)), timeZone);
      const list = await fetchApiSportsGamesByDate(dateKey, season, SPORTSDATA_CACHE_TTL.scores);
      if (list.length) {
        games = list;
        nextGamesDate = dateKey;
        break;
      }
    }
  }
  ...
  if (!nextGamesDate) {
    const hasToday = games.some((game) => {
      const startMs = getApiSportsStartMs(game);
      if (!Number.isFinite(startMs)) return false;
      const startDate = new Date(startMs);
      return getDateKeyForZone(startDate, timeZone) === todayKeyLocal;
    });
    const hasTomorrow = games.some((game) => {
      const startMs = getApiSportsStartMs(game);
      if (!Number.isFinite(startMs)) return false;
      const startDate = new Date(startMs);
      return getDateKeyForZone(startDate, timeZone) === tomorrowKeyLocal;
    });
    nextGamesDate = hasToday ? todayKeyLocal : (hasTomorrow ? tomorrowKeyLocal : todayKeyLocal);
  }

  const oddsTtlMs = SPORTSDATA_CACHE_TTL.odds;

  const oddsMap = new Map();
  for (const game of games) {
    const gameId = game && game.id ? String(game.id) : null;
    if (!gameId) continue;
    const homeName = game && game.teams && game.teams.home && game.teams.home.name ? game.teams.home.name : '';
    const awayName = game && game.teams && game.teams.away && game.teams.away.name ? game.teams.away.name : '';
    const odds = await fetchApiSportsOddsByGame(gameId, season, homeName, awayName, oddsTtlMs);
  ...
  return games.reduce((acc, game) => {
    ...
    const homeTeam = game && game.teams && game.teams.home && game.teams.home.name ? game.teams.home.name : null;
    const awayTeam = game && game.teams && game.teams.away && game.teams.away.name ? game.teams.away.name : null;
    if (!homeTeam || !awayTeam) return acc;
    const odds = oddsMap.get(gameId) || {
      moneyline: { home: null, away: null, draw: null },
      spread: { points: null, home: null, away: null, homePayout: null, awayPayout: null },
      totals: { points: null, over: null, under: null }
    };
    const startTime = game && game.date && game.date.start
      ? game.date.start
      : (game && game.date ? game.date : null);
    acc.push({
      sport: 'NBA',
      gameId,
      startTime: startTime,
...
async function buildNbaNextGamesDebugFromApiSports() {
  const season = getApiSportsNbaSeason();
  const lookaheadDays = 14;
  let nextGamesDate = null;
  let games = [];
  const dateTried = [];
  for (let offset = 0; offset <= lookaheadDays; offset += 1) {
    const dateKey = getUtcDateKey(new Date(Date.now() + (offset * 24 * 60 * 60 * 1000)));
    dateTried.push(dateKey);
    const list = await fetchApiSportsGamesByDate(dateKey, season);
    if (list.length) {
      nextGamesDate = dateKey;
      games = list;
      break;
  ...
  if (!games.length || !nextGamesDate) {
    return { season, source: 'api-sports', scheduleCount: 0, nextGamesDate: null, oddsCount: 0, dateTried, error: null };
  }
  let oddsCount = 0;
  for (const game of games) {
    const gameId = game && game.id ? String(game.id) : null;
    ...
    const homeName = game && game.teams && game.teams.home && game.teams.home.name ? game.teams.home.name : '';
    const awayName = game && game.teams && game.teams.away && game.teams.away.name ? game.teams.away.name : '';
    const odds = await fetchApiSportsOddsByGame(gameId, season, homeName, awayName);
    if (odds && odds.moneyline && (odds.moneyline.home || odds.moneyline.away || odds.moneyline.draw)) oddsCount += 1;
  }
  return {
    season,
    source: 'api-sports',
    scheduleCount: games.length,
    nextGamesDate,
    oddsCount,
    dateTried,
    error: null
...
const allowedOrigins = [
  ...
  'https://pickr-backend-972106331799.us-central1.run.app',
  'https://pickr-backend-v2-972106331799.us-central1.run.app'
];
app.use(cors({
  origin: function(origin, callback) {
    // Allow requests with no origin (like mobile apps or curl requests)
    if (!origin) return callback(null, true);
    if (allowedOrigins.indexOf(origin) !== -1 || origin.endsWith('.netlify.app') || origin.endsWith('.run.app')) {
      callback(null, true);
    } else {
...
// Serve the public folder as static files so the frontend works on localhost
...
function computeDisplayOdds(event) {
  ...
  const getUpdateMs = (obj) => {
    if (!obj) return 0;
    const raw = readStat(obj, ['last_update', 'lastUpdate', 'updated_at', 'updatedAt', 'timestamp', 'date']);
    if (!raw) return 0;
    if (typeof raw === 'number') return raw > 1e12 ? raw : raw * 1000;
    const parsed = Date.parse(String(raw));
    return Number.isFinite(parsed) ? parsed : 0;
  ...
  (event.bookmakers || []).forEach(book => {
    const bookUpdated = getUpdateMs(book);
    const m = book.markets.find(mk => mk.key === 'h2h');
    if (!m) return;
    const marketUpdated = Math.max(bookUpdated, getUpdateMs(m));
    (m.outcomes || []).forEach(outcome => {
      const outcomeUpdated = Math.max(marketUpdated, getUpdateMs(outcome));
      const dec = toDecimal(outcome.price, oddsFormat);
      if (!dec) return;
      if (outcome.name === event.home_team) {
        if (!bestHome || outcomeUpdated >= homeUpdated) {
          bestHome = dec;
          homeUpdated = outcomeUpdated;
        }
      } else if (outcome.name === event.away_team) {
        if (!bestAway || outcomeUpdated >= awayUpdated) {
          bestAway = dec;
          awayUpdated = outcomeUpdated;
        }
      } else if (outcome.name.toLowerCase().includes('draw') || outcome.name.toLowerCase().includes('tie')) {
        if (!bestDraw || outcomeUpdated >= drawUpdated) {
          bestDraw = dec;
          drawUpdated = outcomeUpdated;
        }
      }
    });
  });
...
function buildApiSportsOddsEvent(game, sport) {
  if (!game) return null;
  const homeName = game.homeTeam || game.home_team || null;
  const awayName = game.awayTeam || game.away_team || null;
  const startIso = game.startTime || game.start_time || game.commence_time || null;
  if (!homeName || !awayName || !startIso) return null;
  const moneyline = game.moneyline || (game.odds && game.odds.moneyline) || {};
  const displayOdds = {
    home: normalizeOddsValue(moneyline.home),
    away: normalizeOddsValue(moneyline.away)
  };
  const drawOdd = normalizeOddsValue(moneyline.draw);
  if (drawOdd) displayOdds.draw = drawOdd;
  if (!displayOdds.home && !displayOdds.away && !displayOdds.draw) return null;
  const id = String(game.gameId || game.id || `${sport}-${homeName}-${awayName}-${startIso}`);
  return {
    id,
    match_id: id,
    commence_time: startIso,
...
async function fetchLatestPicks() {
  ...
  const nowMs = Date.now();
  const windowMs = 3 * 24 * 60 * 60 * 1000; // 3 days to cover ESPN multi-day data
  const desiredSports = ['nba', 'nfl', 'nhl', 'soccer', 'mlb'];
  ...
  if (APISPORTS_KEY) {
    try {
      const nbaGames = await buildNbaNextGamesFromApiSports();
      const nbaEvents = nbaGames
        .map(game => buildApiSportsOddsEvent(game, 'nba'))
        .filter(Boolean);
      events = events.concat(nbaEvents);
    } catch (err) {
      console.warn('Warning: failed to fetch odds from API-Sports. Error:', err && err.message);
    }
  }
  if (SPORTSDATAIO_KEY) {
    try {
      ...
      for (const sport of desiredSports) {
        if (sport === 'nba' && APISPORTS_KEY) continue;
        for (const dateKey of dateKeys) {
      ...
      events = events.concat(sportsDataEvents);
    } catch (err) {
      console.warn('Warning: failed to fetch odds from SportsDataIO. Error:', err && err.message);
    }
  }

  // Fallback: if no events from primary sources, try Odds API then ESPN
  if (!events.length) {
    console.log('fetchLatestPicks: primary sources empty, trying Odds API…');
    for (const sport of desiredSports) {
      try {
        const oddsGames = await fetchGamesFromOddsApi(sport);
        if (oddsGames && oddsGames.length) {
          const oddsEvents = oddsGames.map(g => ({
            id: g.gameId || `${g.sport_key}-${g.homeTeam}-${g.awayTeam}`,
            commence_time: g.startTime,
            home_team: g.homeTeam,
            away_team: g.awayTeam,
            sport_key: g.sport_key || sport,
            league: g.league || String(sport).toUpperCase(),
            displayOdds: g.moneyline || {}
          }));
          events = events.concat(oddsEvents);
        }
      } catch (e) { console.warn(`Odds API fallback failed for ${sport} in fetchLatestPicks:`, e && e.message); }
    }
  }

  if (!events.length) {
    console.log('fetchLatestPicks: Odds API empty too, trying ESPN scoreboard…');
    for (const sport of desiredSports) {
      try {
        const espnGames = await fetchGamesFromEspn(sport);
        if (espnGames && espnGames.length) {
          const espnEvents = espnGames.map(g => ({
            id: g.gameId || `espn-${g.homeTeam}-${g.awayTeam}`,
            commence_time: g.startTime,
            home_team: g.homeTeam,
            away_team: g.awayTeam,
            sport_key: g.sport_key || sport,
          ...
          events = events.concat(espnEvents);
        }
      } catch (e) { console.warn(`ESPN fallback failed for ${sport} in fetchLatestPicks:`, e && e.message); }
    }
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// The Odds API — universal game/odds fallback for all sports
// ─────────────────────────────────────────────────────────────────────────────
...
const oddsApiCache = new Map();
const ODDS_API_CACHE_TTL = 5 * 60 * 1000; // 5 minutes

function getOddsApiCached(key) {
  const entry = oddsApiCache.get(key);
  if (entry && Date.now() < entry.expiresAt) return entry.value;
  if (entry) oddsApiCache.delete(key);
  return null;
}

function setOddsApiCached(key, value, ttlMs) {
  oddsApiCache.set(key, { value, expiresAt: Date.now() + ttlMs });
}

function extractBestOdds(bookmakers, homeName, awayName) {
  const ml = { home: null, away: null, draw: null };
  if (!Array.isArray(bookmakers) || !bookmakers.length) return { moneyline: ml };
  // Prefer FanDuel > DraftKings > first available
  const preferred = ['fanduel', 'draftkings', 'bovada', 'betmgm'];
  let chosen = null;
  for (const pref of preferred) {
    chosen = bookmakers.find(b => b.key === pref);
    if (chosen) break;
  }
  if (!chosen) chosen = bookmakers[0];
  const h2h = chosen && chosen.markets && chosen.markets.find(m => m.key === 'h2h');
  if (h2h && Array.isArray(h2h.outcomes)) {
    const homeNorm = String(homeName || '').toLowerCase();
    const awayNorm = String(awayName || '').toLowerCase();
    h2h.outcomes.forEach(o => {
      const name = String(o.name || '').toLowerCase();
      if (name === 'draw' || name === 'tie') { ml.draw = o.price; return; }
      if (name === homeNorm) ml.home = o.price;
      else if (name === awayNorm) ml.away = o.price;
    });
    // If name matching failed, fall back to positional
    if (ml.home === null && ml.away === null && h2h.outcomes.length >= 2) {
      ml.home = h2h.outcomes[0].price;
      ml.away = h2h.outcomes[1].price;
      if (h2h.outcomes[2]) ml.draw = h2h.outcomes[2].price;
    }
  }
  return { moneyline: ml, spread: {}, totals: {} };
}

async function fetchGamesFromOddsApi(sport) {
  const apiKey = getOddsApiKey();
  if (!apiKey) return [];
  const sportKeys = ODDS_API_SPORT_KEYS[String(sport).toLowerCase()];
  if (!sportKeys || !sportKeys.length) return [];

  const allGames = [];
  for (const sportKey of sportKeys) {
    const cacheKey = `oddsapi:${sportKey}`;
    const cached = getOddsApiCached(cacheKey);
    if (cached) {
      allGames.push(...cached);
      continue;
    }
    try {
      const resp = await axios.get(`https://api.the-odds-api.com/v4/sports/${encodeURIComponent(sportKey)}/odds`, {
        params: {
          apiKey,
          regions: 'us',
          markets: 'h2h',
      ...
      const leagueLabel = ODDS_API_LEAGUE_LABELS[sportKey] || '';
      const normalized = events.map(ev => {
        const odds = extractBestOdds(ev.bookmakers, ev.home_team, ev.away_team);
        return {
          sport: String(sport).toUpperCase(),
          league: leagueLabel || String(ev.sport_title || sport).toUpperCase(),
          gameId: String(ev.id || ''),
          startTime: ev.commence_time || null,
      ...
      setOddsApiCached(cacheKey, normalized, ODDS_API_CACHE_TTL);
      allGames.push(...normalized);
    } catch (err) {
      console.warn(`Odds API fetch failed for ${sportKey}:`, err && err.message);
    }
  }
  return allGames;
}
...
// ─────────────────────────────────────────────────────────────────────────────
// ESPN Scoreboard — free fallback (no API key required)
// ─────────────────────────────────────────────────────────────────────────────
...
const ESPN_SCOREBOARD_TTL = 5 * 60 * 1000; // 5 minutes

async function fetchGamesFromEspn(sport) {
  const paths = ESPN_SCOREBOARD_ENDPOINTS[String(sport).toLowerCase()];
  if (!paths || !paths.length) return [];

  const allGames = [];
  // Determine how many days forward to fetch
  const daysToFetch = sport === 'soccer' ? 5 : (sport === 'nfl' ? 10 : 3);
  ...
  for (const path of paths) {
    for (let dayOffset = 0; dayOffset < daysToFetch; dayOffset++) {
      ...
      const cacheKey = `espn-sb:${path}:${dateStr}`;

      // Check cache
      const cachedEntry = espnScoreboardCache.get(cacheKey);
      if (cachedEntry && Date.now() < cachedEntry.expiresAt) {
        allGames.push(...cachedEntry.value);
        continue;
      }
      ...
      try {
        ...
        const events = (resp.data && Array.isArray(resp.data.events)) ? resp.data.events : [];
        const leagueLabel = ESPN_LEAGUE_LABELS[path] || String(sport).toUpperCase();
        ...
        for (const ev of events) {
          ...
          const stateStr = (comp.status && comp.status.type && comp.status.type.state) || '';
          if (stateStr === 'post') continue; // game already finished

          const homeComp = (comp.competitors || []).find(c => c.homeAway === 'home') || {};
          const awayComp = (comp.competitors || []).find(c => c.homeAway === 'away') || {};
          const homeTeam = (homeComp.team && homeComp.team.displayName) || '';
          const awayTeam = (awayComp.team && awayComp.team.displayName) || '';
          if (!homeTeam || !awayTeam) continue;

          // Extract moneyline odds
          const oddsData = (comp.odds && comp.odds[0]) || {};
          const mlData = oddsData.moneyline || {};
          const homeML = mlData.home && mlData.home.close && mlData.home.close.odds ? Number(mlData.home.close.odds) : null;
          const awayML = mlData.away && mlData.away.close && mlData.away.close.odds ? Number(mlData.away.close.odds) : null;
          const drawML = mlData.draw && mlData.draw.close && mlData.draw.close.odds ? Number(mlData.draw.close.odds) : null;

          // Extract spread
          const psData = oddsData.pointSpread || {};
          const homeSpreadLine = psData.home && psData.home.close && psData.home.close.line ? Number(psData.home.close.line) : null;
          const awaySpreadLine = psData.away && psData.away.close && psData.away.close.line ? Number(psData.away.close.line) : null;
          const homeSpreadOdds = psData.home && psData.home.close && psData.home.close.odds ? Number(psData.home.close.odds) : null;
          const awaySpreadOdds = psData.away && psData.away.close && psData.away.close.odds ? Number(psData.away.close.odds) : null;

          // Extract totals
          const totData = oddsData.total || {};
          const overLine = totData.over && totData.over.close && totData.over.close.line ? String(totData.over.close.line).replace(/^[ou]/i, '') : null;
          const overOdds = totData.over && totData.over.close && totData.over.close.odds ? Number(totData.over.close.odds) : null;
          const underOdds = totData.under && totData.under.close && totData.under.close.odds ? Number(totData.under.close.odds) : null;

          const moneyline = {};
          if (Number.isFinite(homeML)) moneyline.home = homeML;
          if (Number.isFinite(awayML)) moneyline.away = awayML;
          if (Number.isFinite(drawML)) moneyline.draw = drawML;

          const spread = {};
          if (Number.isFinite(homeSpreadLine)) { spread.home = homeSpreadOdds; spread.homeLine = homeSpreadLine; }
          if (Number.isFinite(awaySpreadLine)) { spread.away = awaySpreadOdds; spread.awayLine = awaySpreadLine; }

          const totals = {};
          if (overLine) { totals.overUnder = Number(overLine) || oddsData.overUnder || null; totals.over = overOdds; totals.under = underOdds; }
        ...
        espnScoreboardCache.set(cacheKey, { value: normalized, expiresAt: Date.now() + ESPN_SCOREBOARD_TTL });
        allGames.push(...normalized);
      } catch (err) {
        console.warn(`ESPN scoreboard fetch failed for ${path} ${dateStr}:`, err && err.message);
      }
    }
  }

  console.log(`ESPN scoreboard fallback for ${sport}: ${allGames.length} games`);
  return allGames;
}
...
function pickMatchesTeam(pick, teamValues) {
  const pickToken = normalizeToken(pick);
  if (!pickToken) return false;
  return teamValues.some((value) => {
    const token = normalizeToken(value);
    return token && (pickToken === token || pickToken.includes(token) || token.includes(pickToken));
  });
}

function extractSportsDataScores(game) {
  if (!game) return null;
  const home = readStat(game, ['HomeTeamName', 'HomeTeam', 'HomeTeamKey', 'HomeTeamAbbreviation']);
  const away = readStat(game, ['AwayTeamName', 'AwayTeam', 'AwayTeamKey', 'AwayTeamAbbreviation']);
  if (!home || !away) return null;
...
function isApiSportsFinal(game) {
  const status = game && game.status ? game.status : (game && game.game && game.game.status ? game.game.status : null);
  const shortCode = status && status.short ? String(status.short).toUpperCase() : '';
  const longCode = status && status.long ? String(status.long).toLowerCase() : '';
  if (shortCode && ['FT', 'AOT', 'AET', 'FINAL', 'F'].includes(shortCode)) return true;
  if (longCode && longCode.includes('final')) return true;
  return false;
}

function extractApiSportsScores(game) {
  if (!game) return null;
  const teams = game.teams || {};
  const homeTeam = teams.home || {};
  const awayTeam = teams.away || {};
  const home = homeTeam.name || homeTeam.nickname || homeTeam.code || null;
  const away = awayTeam.name || awayTeam.nickname || awayTeam.code || null;
  if (!home || !away) return null;
  const scores = game.scores || {};
  const homeScore = Number((scores.home && (scores.home.total || scores.home.points || scores.home.score)) || scores.home || null);
  const awayScore = Number((scores.away && (scores.away.total || scores.away.points || scores.away.score)) || scores.away || null);
  if (!Number.isFinite(homeScore) || !Number.isFinite(awayScore)) return null;
  return { home, away, homeScore, awayScore, homeCode: homeTeam.code, awayCode: awayTeam.code };
}

function determineOutcomeFromApiSports(selection, game) {
  if (!selection || !game) return 'pending';
  if (!isApiSportsFinal(game)) return 'pending';
  const scores = extractApiSportsScores(game);
  if (!scores) return 'pending';
  if (scores.homeScore === scores.awayScore) {
    return normalizeToken(selection.pick) === 'draw' ? 'win' : 'lose';
  }
  const winner = scores.homeScore > scores.awayScore ? scores.home : scores.away;
  const winnerValues = winner === scores.home
    ? [scores.home, scores.homeCode]
    : [scores.away, scores.awayCode];
  return pickMatchesTeam(selection.pick, winnerValues) ? 'win' : 'lose';
}

function findApiSportsGameByTeams(selection, games) {
  if (!selection || !Array.isArray(games) || !games.length) return null;
  const selHome = selection.homeTeam || selection.home_team || '';
  const selAway = selection.awayTeam || selection.away_team || '';
  if (!selHome || !selAway) return null;
  return games.find((game) => {
    const teams = game && game.teams ? game.teams : {};
    const homeTeam = teams.home || {};
    const awayTeam = teams.away || {};
    const homeName = homeTeam.name || homeTeam.nickname || homeTeam.code || '';
    const awayName = awayTeam.name || awayTeam.nickname || awayTeam.code || '';
    const homeCode = homeTeam.code || '';
    const awayCode = awayTeam.code || '';
    return pickMatchesTeam(selHome, [homeName, homeCode]) && pickMatchesTeam(selAway, [awayName, awayCode]);
  }) || null;
}

function findApiSportsGameByPick(selection, games) {
  if (!selection || !Array.isArray(games) || !games.length) return null;
  const pick = selection.pick || '';
  if (!pick) return null;
  const targetTime = selection.commenceTime || selection.startTime || selection.commence_time || selection.start_time || selection.date || null;
  ...
  const matches = games.filter((game) => {
    ...
    const homeCode = homeTeam.code || '';
    const awayCode = awayTeam.code || '';
    return pickMatchesTeam(pick, [homeName, homeCode]) || pickMatchesTeam(pick, [awayName, awayCode]);
  });
  if (!matches.length) return null;
  if (!Number.isFinite(targetMs)) return matches[0];
  let best = matches[0];
  let bestDelta = Infinity;
  matches.forEach((game) => {
    const startRaw = game && game.date && game.date.start
      ? game.date.start
      : (game && game.date ? game.date : (game && game.dateTime ? game.dateTime : null));
...
function determineOutcomeFromSportsData(selection, game) {
  if (!selection || !game) return 'pending';
  if (!isFinalGame(game)) return 'pending';
  const scores = extractSportsDataScores(game);
  if (!scores) return 'pending';
  if (scores.homeScore === scores.awayScore) {
    return normalizeToken(selection.pick) === 'draw' ? 'win' : 'lose';
  }
  const winner = scores.homeScore > scores.awayScore ? scores.home : scores.away;
  const homeValues = [scores.home, readStat(game, ['HomeTeamKey', 'HomeTeamAbbreviation'])];
  const awayValues = [scores.away, readStat(game, ['AwayTeamKey', 'AwayTeamAbbreviation'])];
  const winnerValues = winner === scores.home ? homeValues : awayValues;
  return pickMatchesTeam(selection.pick, winnerValues) ? 'win' : 'lose';
}
...
async function resolveBets(bets, { allowSettle = true } = {}) {
  if (!firestore) return 0;
  if (!Array.isArray(bets) || bets.length === 0) return 0;
  const apiKey = getOddsApiKey();
  if (!apiKey) {
    console.warn('resolveBets skipped: ODDS_API_KEY not configured');
    return 0;
  }
  ...
  bets.forEach((bet) => {
    (bet.selections || []).forEach((sel) => {
      ...
      if (!eventId) return;
      const sportKeyRaw = sel.sportKey || sel.sport_key || null;
      const sportKey = sportKeyRaw ? String(sportKeyRaw).toLowerCase() : null;
  ...
  const sportToDateKeys = new Map();
  const sportsMissingDates = new Set();
  const addDateKey = (sportKey, dateKey) => {
    if (!sportKey || !dateKey) return;
    const set = sportToDateKeys.get(sportKey) || new Set();
    set.add(dateKey);
    sportToDateKeys.set(sportKey, set);
  };

  const addNeighborDates = (sportKey, dateKey) => {
    if (!sportKey || !dateKey) return;
    const base = new Date(`${dateKey}T00:00:00Z`);
    if (Number.isNaN(base.getTime())) return;
    const prev = new Date(base.getTime() - 24 * 60 * 60 * 1000);
    const next = new Date(base.getTime() + 24 * 60 * 60 * 1000);
    addDateKey(sportKey, getUtcDateKey(prev));
    addDateKey(sportKey, getUtcDateKey(next));
  };

  const getSelectionDateKey = (selection, bet) => {
    const time = selection && (selection.commenceTime || selection.startTime)
      ? selection.commenceTime || selection.startTime
      : (bet && (bet.placedAt || bet.date || bet.createdAt));
  ...
  for (const [sportKey] of sportToEventIds.entries()) {
  ...
  bets.forEach((bet) => {
    (bet.selections || []).forEach((selection) => {
      const sportKeyRaw = selection && (selection.sportKey || selection.sport_key);
      const sportKey = sportKeyRaw ? String(sportKeyRaw).toLowerCase() : null;
      const eventId = selection && selection.eventId;
      if (!sportKey || !eventId) return;
      if (resultsByEvent.has(eventId)) return;
      const dateKey = getSelectionDateKey(selection, bet);
      if (dateKey) {
        addDateKey(sportKey, dateKey);
        if (sportKey === 'nba') addNeighborDates(sportKey, dateKey);
      } else {
        sportsMissingDates.add(sportKey);
      }
    });
  });

  if (sportsMissingDates.size) {
    const now = new Date();
    for (let offset = 0; offset <= 7; offset += 1) {
      const date = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - offset));
      const dateKey = getUtcDateKey(date);
      sportsMissingDates.forEach((sportKey) => addDateKey(sportKey, dateKey));
    }
  }

  const sportsDataResultsByKey = new Map();
  if (SPORTS_DATA_IO_KEY) {
    for (const [sportKey, dateKeys] of sportToDateKeys.entries()) {
      for (const dateKey of dateKeys) {
        try {
          const payload = await fetchSportsDataIo(sportKey, `/scores/json/GamesByDate/${dateKey}`, null, SPORTSDATA_CACHE_TTL.scores);
          const games = Array.isArray(payload) ? payload : [];
          games.forEach((game) => {
            const gameId = readStat(game, ['GameID', 'GameId', 'GameKey']);
            if (!gameId) return;
            sportsDataResultsByKey.set(`${sportKey}:${String(gameId)}`, game);
          });
        } catch (err) {
          console.warn('Failed to fetch SportsData scores for', sportKey, dateKey, err && err.message);
        }
      }
    }
  }

  const apiSportsResultsByKey = new Map();
  const apiSportsGamesByDate = new Map();
  if (APISPORTS_KEY) {
    const season = getApiSportsNbaSeason();
    const nbaDates = sportToDateKeys.get('nba') || new Set();
    for (const dateKey of nbaDates) {
      try {
        const games = await fetchApiSportsGamesByDate(dateKey, season, SPORTSDATA_CACHE_TTL.scores);
        apiSportsGamesByDate.set(dateKey, games);
        games.forEach((game) => {
          const gameId = game && game.id ? String(game.id) : null;
          if (!gameId) return;
          apiSportsResultsByKey.set(`nba:${gameId}`, game);
        });
      } catch (err) {
        console.warn('Failed to fetch ApiSports scores for', dateKey, err && err.message);
      }
    }
  }
  ...
  for (const bet of bets) {
    ...
    const outcomes = selections.map((sel) => {
      const oddsResult = resultsByEvent.get(sel.eventId);
      let outcome = determineOutcome(sel, oddsResult);
      if (outcome !== 'pending') return outcome;
      const sportKeyRaw = sel.sportKey || sel.sport_key || null;
      const sportKey = sportKeyRaw ? String(sportKeyRaw).toLowerCase() : null;
      const key = sportKey ? `${sportKey}:${String(sel.eventId)}` : null;
      if (key && sportsDataResultsByKey.has(key)) {
        outcome = determineOutcomeFromSportsData(sel, sportsDataResultsByKey.get(key));
      }
      if (outcome === 'pending' && key && apiSportsResultsByKey.has(key)) {
        outcome = determineOutcomeFromApiSports(sel, apiSportsResultsByKey.get(key));
      }
      if (outcome === 'pending' && sportKey === 'nba') {
        const dateKey = getSelectionDateKey(sel, bet);
        const games = dateKey ? apiSportsGamesByDate.get(dateKey) : null;
        let matched = findApiSportsGameByTeams(sel, games || []);
        if (!matched) matched = findApiSportsGameByPick(sel, games || []);
        if (matched) outcome = determineOutcomeFromApiSports(sel, matched);
      }
      return outcome;
    });

    const updatedSelections = selections.map((sel, index) => {
      const status = outcomes[index] || 'pending';
      if (sel && sel.status === status) return sel;
      return Object.assign({}, sel, { status });
    });
    const hasSelectionUpdates = updatedSelections.some((sel, index) => sel !== selections[index]);
    if (hasSelectionUpdates) {
      try {
        await firestore.collection('bets').doc(bet.betId || bet.id).update({ selections: updatedSelections });
      } catch (e) {
        console.warn('Failed to update bet selections', bet.betId || bet.id, e && e.message);
      }
    }

    if (!allowSettle || String(bet.status || '').toLowerCase() !== 'pending') continue;
    let status = null;
...
async function settlePendingBets() {
  ...
  try {
    ...
    const settledCount = await resolveBets(pendingBets, { allowSettle: true });
...
app.get('/api/leaderboard/players', async (req, res) => {
  try {
    ...
    snap.forEach((doc) => {
      const u = doc.data() || {};
      if (!u.profileComplete || !u.ageVerified) return;
      const screenName = String(u.screenName || '').trim();
      if (!screenName) return;
      if (screenName.includes('@')) return;
      if (!/^[A-Z0-9_.]+$/.test(screenName)) return;
      const emailPrefix = String(u.email || '').split('@')[0] || '';
      if (emailPrefix && screenName.toLowerCase() === emailPrefix.toLowerCase()) return;
      rows.push({
        uid: u.uid || doc.id,
        name: screenName,
...
async function fetchGamesByDateFallback(sport, maxDays = 30) {
  const now = new Date();
  const cursor = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
  let lastError = null;
  for (let offset = 0; offset <= maxDays; offset += 1) {
    const dateKey = getUtcDateKey(new Date(cursor.getTime() + (offset * 24 * 60 * 60 * 1000)));
    try {
      const [scoresPayload, oddsPayload] = await Promise.all([
        fetchSportsDataIo(sport, `/scores/json/GamesByDate/${dateKey}`, null, SPORTSDATA_CACHE_TTL.scores),
        fetchSportsDataIo(sport, `/odds/json/GameOddsByDate/${dateKey}`, null, SPORTSDATA_CACHE_TTL.odds)
      ...
      return { games, oddsMap: buildOddsMap(odds), dateKey };
    } catch (err) {
      lastError = err;
      // Ignore missing dates; keep scanning.
    }
  }
  return { games: [], oddsMap: new Map(), dateKey: null, error: lastError };
}

function getAxiosErrorInfo(err) {
  if (!err) return null;
...
async function buildNextGamesDebugForSport(sport) {
  const todayKey = getUtcDateKey(new Date());
  ...
  try {
    const [scoresPayload, oddsPayload] = await Promise.all([
      fetchSportsDataIo(sport, `/scores/json/GamesByDate/${todayKey}`, null, SPORTSDATA_CACHE_TTL.scores),
      fetchSportsDataIo(sport, `/odds/json/GameOddsByDate/${todayKey}`, null, SPORTSDATA_CACHE_TTL.odds)
    ]);
    todayGames = Array.isArray(scoresPayload) ? scoresPayload : [];
    todayOdds = Array.isArray(oddsPayload) ? oddsPayload : [];
  } catch (err) {
    lastError = err;
  }
  ...
  try {
    const [scoresPayload, oddsPayload] = await Promise.all([
      fetchSportsDataIo(sport, `/scores/json/GamesByDate/${tomorrowKey}`, null, SPORTSDATA_CACHE_TTL.scores),
      fetchSportsDataIo(sport, `/odds/json/GameOddsByDate/${tomorrowKey}`, null, SPORTSDATA_CACHE_TTL.odds)
    ]);
    tomorrowGames = Array.isArray(scoresPayload) ? scoresPayload : [];
    tomorrowOdds = Array.isArray(oddsPayload) ? oddsPayload : [];
  } catch (err) {
    lastError = err;
  }

  const fallback = await fetchGamesByDateFallback(sport);
  if (fallback.error) lastError = fallback.error;

  return {
    sport,
    today: { date: todayKey, games: todayGames.length, odds: todayOdds.length },
    tomorrow: { date: tomorrowKey, games: tomorrowGames.length, odds: tomorrowOdds.length },
    fallback: { date: fallback.dateKey, games: fallback.games.length },
    error: getAxiosErrorInfo(lastError)
  };
}
...
async function buildGamesResponse(sport, options = {}) {
  ...
  let games = [];
  let oddsMap = new Map();
  let forceNextGameDay = false;
  let nextGameDayKey = null;
  const startCutoffMs = Number(options.startCutoffMs || 0);
  ...
  if (!games.length) {
    ...
    oddsMap = fallback.oddsMap;
    if (fallback.dateKey) {
      forceNextGameDay = true;
      nextGameDayKey = fallback.dateKey;
    }
  }
  ...
  if (options.nextGameDayOnly && !forceNextGameDay) {
    let minStart = null;
    for (const game of games) {
  ...
  if (!options.nextGameDayOnly && !forceNextGameDay) {
    ...
    for (const game of games) {
      ...
      if (startMs >= now && startMs <= now + windowMs) hasUpcoming = true;
      if (startMs >= now && (minFuture === null || startMs < minFuture)) minFuture = startMs;
    }
    if (!hasUpcoming && minFuture !== null) {
      forceNextGameDay = true;
      nextGameDayKey = getUtcDateKey(new Date(minFuture));
    }
  }
  ...
  for (const game of games) {
    ...
    if (!Number.isFinite(startMs)) continue;
    if (options.nextGameDayOnly || forceNextGameDay) {
      if (!nextGameDayKey || startMs < now - startCutoffMs) continue;
      const gameDayKey = getUtcDateKey(new Date(startMs));
      if (gameDayKey !== nextGameDayKey) continue;
    } else if (startMs < now - startCutoffMs || startMs > now + windowMs) {
      continue;
    }
...
async function buildNbaNextGamesResponse() {
  if (APISPORTS_KEY) {
    return buildNbaNextGamesFromApiSports();
  }
  const { schedule } = await fetchNbaScheduleWithFallback();
  let effectiveSchedule = schedule;
  let nextGamesDateOverride = null;
  if (!effectiveSchedule.length) {
    const dateFallback = await fetchNbaGamesByDateFallback();
    effectiveSchedule = dateFallback.games;
    nextGamesDateOverride = dateFallback.nextGamesDate;
  }
  if (!effectiveSchedule.length) return [];

  const gamesByDate = new Map();
  let lastDateMs = null;
  effectiveSchedule.forEach((game) => {
    const startIso = getGameStartIso(game);
    const startMs = startIso ? Date.parse(startIso) : NaN;
    if (!Number.isFinite(startMs)) return;
    const dateKey = getUtcDateKey(new Date(startMs));
    const bucket = gamesByDate.get(dateKey) || [];
    bucket.push(game);
    gamesByDate.set(dateKey, bucket);
    if (lastDateMs === null || startMs > lastDateMs) lastDateMs = startMs;
  });

  if (!gamesByDate.size || lastDateMs === null) return [];

  const now = new Date();
  const nowMs = now.getTime();
  const NBA_GAME_DURATION_MS = 4 * 60 * 60 * 1000;
  // Games that already started are NOT bettable, but we still consider
  // a date as having upcoming games when at least one hasn't finished yet
  // (so the day remains visible while some games are still playing).
  const cursor = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
  let cursorMs = cursor.getTime();
  const endMs = new Date(Date.UTC(new Date(lastDateMs).getUTCFullYear(), new Date(lastDateMs).getUTCMonth(), new Date(lastDateMs).getUTCDate())).getTime();
  let nextGamesDate = nextGamesDateOverride;
  while (cursorMs <= endMs) {
    const dateKey = getUtcDateKey(new Date(cursorMs));
    const bucket = gamesByDate.get(dateKey) || [];
    const hasUpcoming = bucket.some((game) => {
      const startIso = getGameStartIso(game);
      const startMs = startIso ? Date.parse(startIso) : NaN;
      if (!Number.isFinite(startMs)) return false;
      return startMs + NBA_GAME_DURATION_MS >= nowMs;
    });
    if (bucket.length && hasUpcoming) {
      nextGamesDate = dateKey;
      break;
    }
  ...
  if (!nextGamesDate) return [];

  const oddsPayload = await fetchSportsData('nba', `/odds/json/GameOddsByDate/${nextGamesDate}`, null, SPORTSDATA_CACHE_TTL.odds);
  const oddsMap = buildOddsMap(Array.isArray(oddsPayload) ? oddsPayload : []);

  const nextGames = nextGamesDateOverride ? effectiveSchedule : (gamesByDate.get(nextGamesDate) || []);
  return nextGames.reduce((acc, game) => {
    const gameId = readStat(game, ['GameID', 'GameId', 'GameKey']);
    if (!gameId) return acc;
    const startIso = getGameStartIso(game);
    const startMs = startIso ? Date.parse(startIso) : NaN;
    // Exclude games that have already started — they are no longer bettable.
    // We keep the "duration" check only for the day-selection logic above;
    // individual games are hidden the moment their start time passes.
    if (Number.isFinite(startMs) && startMs <= nowMs) return acc;
    const homeTeam = readStat(game, ['HomeTeamName', 'HomeTeam', 'HomeTeamKey', 'HomeTeamAbbreviation']);
    const awayTeam = readStat(game, ['AwayTeamName', 'AwayTeam', 'AwayTeamKey', 'AwayTeamAbbreviation']);
    if (!homeTeam || !awayTeam) return acc;
    const odds = oddsMap.get(String(gameId)) || { moneyline: {}, spread: {}, totals: {} };
    acc.push({
      sport: 'NBA',
      gameId: String(gameId),
      startTime: startIso,
...
function getNbaScheduleSeasonKeys() {
  const primary = Number(getNbaSeasonYear());
  const years = [primary, primary - 1].filter((v, i, a) => a.indexOf(v) === i);
  const suffixes = ['', 'REG', 'POST', 'PRE'];
  const keys = [];
  const addKey = (value) => {
    if (!value) return;
  ...
  years.forEach((year) => {
    suffixes.forEach((suffix) => {
      const key = suffix ? `${year}${suffix}` : String(year);
      addKey(key);
    });
  });
  years.forEach((year) => {
    const prev = year - 1;
    addKey(`${prev}-${String(year).slice(-2)}`);
    addKey(`${prev}-${year}`);
    addKey(`${year}-${String(year + 1).slice(-2)}`);
    addKey(`${year}-${year + 1}`);
  });
  return keys;
}

async function fetchNbaScheduleWithFallback() {
  const seasonKeys = getNbaScheduleSeasonKeys();
  let lastError = null;
  for (const seasonKey of seasonKeys) {
    try {
      const payload = await fetchSportsData('nba', `/scores/json/Schedule/${seasonKey}`, null, 6 * 60 * 60 * 1000);
      const schedule = Array.isArray(payload) ? payload : [];
      if (schedule.length) return { season: String(seasonKey), schedule, error: null, seasonTried: seasonKeys };
    } catch (err) {
      lastError = err;
    }
  }
  return { season: seasonKeys[0] || String(getNbaSeasonYear()), schedule: [], error: lastError, seasonTried: seasonKeys };
}

async function fetchNbaGamesByDateFallback(maxDays = 14) {
  const now = new Date();
  const cursor = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
  const tried = [];
  let lastError = null;
  for (let offset = 0; offset <= maxDays; offset += 1) {
    ...
    tried.push(dateKey);
    try {
      const payload = await fetchSportsData('nba', `/scores/json/GamesByDate/${dateKey}`, null, SPORTSDATA_CACHE_TTL.scores);
      const games = Array.isArray(payload) ? payload : [];
      if (games.length) return { nextGamesDate: dateKey, games, error: null, dateTried: tried };
    } catch (err) {
      lastError = err;
    }
  }
  return { nextGamesDate: null, games: [], error: lastError, dateTried: tried };
}

async function buildNbaNextGamesDebug() {
  if (APISPORTS_KEY) {
    return buildNbaNextGamesDebugFromApiSports();
  }
  const scheduleResult = await fetchNbaScheduleWithFallback();
  let schedule = scheduleResult.schedule;
  let season = scheduleResult.season;
  let error = scheduleResult.error;
  let seasonTried = scheduleResult.seasonTried;
  let source = 'schedule';
  let dateTried = [];
  let nextGamesDateOverride = null;
  ...
  if (!schedule.length) {
    const dateFallback = await fetchNbaGamesByDateFallback();
    schedule = dateFallback.games;
    nextGamesDateOverride = dateFallback.nextGamesDate;
    dateTried = dateFallback.dateTried || [];
    error = dateFallback.error;
    source = 'date';
  }

  if (!schedule.length) {
    const status = error && error.response && error.response.status ? error.response.status : null;
    const message = error && error.message ? String(error.message) : null;
    return {
      season,
      seasonTried,
      dateTried,
  ...
  const cursor = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
  let cursorMs = cursor.getTime();
  const endMs = lastDateMs === null
    ? cursorMs
    : new Date(Date.UTC(new Date(lastDateMs).getUTCFullYear(), new Date(lastDateMs).getUTCMonth(), new Date(lastDateMs).getUTCDate())).getTime();
  let nextGamesDate = nextGamesDateOverride;
  while (cursorMs <= endMs) {
    const dateKey = getUtcDateKey(new Date(cursorMs));
    if (gamesByDate.get(dateKey) && gamesByDate.get(dateKey).length) {
      nextGamesDate = dateKey;
      break;
    }
    cursorMs += 24 * 60 * 60 * 1000;
  ...
  if (!nextGamesDate) {
    return { season, seasonTried, dateTried, source, scheduleCount: schedule.length, nextGamesDate: null, oddsCount: 0, sampleGame: null, sampleOdds: null };
  }

  const oddsPayload = await fetchSportsData('nba', `/odds/json/GameOddsByDate/${nextGamesDate}`, null, 2 * 60 * 1000);
  const odds = Array.isArray(oddsPayload) ? oddsPayload : [];
  const sampleGame = nextGamesDateOverride ? schedule[0] || null : (gamesByDate.get(nextGamesDate) || [])[0] || null;
  const sampleOdds = odds[0] || null;
...
app.get('/api/games/nba', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    let games = [];
    try { games = await buildNbaNextGamesResponse(); } catch (e) { console.warn('NBA primary source failed:', e && e.message); }
    if (!games || !games.length) {
      console.log('NBA: falling back to The Odds API');
      try { games = await fetchGamesFromOddsApi('nba'); } catch (e) { console.warn('NBA Odds API fallback failed:', e && e.message); }
    }
    if (!games || !games.length) {
      console.log('NBA: falling back to ESPN scoreboard');
      try { games = await fetchGamesFromEspn('nba'); } catch (e) { console.warn('NBA ESPN fallback failed:', e && e.message); }
    }
...
app.get('/api/games/nba/debug', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    const data = await buildNbaNextGamesDebug();
...
// Public debug endpoint (no auth) to validate SportsDataIO availability without exposing secrets.
app.get('/api/games/nba/debug-public', async (req, res) => {
  try {
    const data = await buildNbaNextGamesDebug();
    const safe = {
      season: data.season,
      seasonTried: data.seasonTried || [],
      dateTried: data.dateTried || [],
      source: data.source || null,
      scheduleCount: data.scheduleCount,
      nextGamesDate: data.nextGamesDate,
...
// Public debug endpoint to inspect the NBA games list without auth.
app.get('/api/games/nba/list-public', async (req, res) => {
  try {
    const games = await buildNbaNextGamesResponse();
    ...
    return res.json({ count: list.length, games: list });
  } catch (err) {
    return sendSportsDataError(res, err);
  }
});

app.get('/api/games/:sport/debug-public', async (req, res) => {
  const sport = normalizeSportsDataSport(req.params.sport);
  if (!sport || sport === 'nba') {
    return res.status(400).json({ error: 'sport must be nfl, nhl, mlb, or soccer' });
  }
  if (!['nfl', 'nhl', 'mlb', 'soccer'].includes(sport)) {
    return res.status(400).json({ error: 'sport must be nfl, nhl, mlb, or soccer' });
  }
  try {
    const data = await buildNextGamesDebugForSport(sport);
    return res.json(data);
  } catch (err) {
...
app.get('/api/games/nfl', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    let games = [];
    try { games = await buildGamesResponse('nfl'); } catch (e) { console.warn('NFL primary source failed:', e && e.message); }
    if (!games || !games.length) {
      console.log('NFL: falling back to The Odds API');
      try { games = await fetchGamesFromOddsApi('nfl'); } catch (e) { console.warn('NFL Odds API fallback failed:', e && e.message); }
    }
    if (!games || !games.length) {
      console.log('NFL: falling back to ESPN scoreboard');
      try { games = await fetchGamesFromEspn('nfl'); } catch (e) { console.warn('NFL ESPN fallback failed:', e && e.message); }
    }
    return res.json(games || []);
  } catch (err) {
    return sendSportsDataIoError(res, err);
...
app.get('/api/games/nhl', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    let games = [];
    try { games = await buildGamesResponse('nhl'); } catch (e) { console.warn('NHL primary source failed:', e && e.message); }
    if (!games || !games.length) {
      console.log('NHL: falling back to The Odds API');
      try { games = await fetchGamesFromOddsApi('nhl'); } catch (e) { console.warn('NHL Odds API fallback failed:', e && e.message); }
    }
    if (!games || !games.length) {
      console.log('NHL: falling back to ESPN scoreboard');
      try { games = await fetchGamesFromEspn('nhl'); } catch (e) { console.warn('NHL ESPN fallback failed:', e && e.message); }
    }
    return res.json(games || []);
  } catch (err) {
    return sendSportsDataIoError(res, err);
...
app.get('/api/games/mlb', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    let games = [];
    try { games = await buildGamesResponse('mlb'); } catch (e) { console.warn('MLB primary source failed:', e && e.message); }
    if (!games || !games.length) {
      console.log('MLB: falling back to The Odds API');
      try { games = await fetchGamesFromOddsApi('mlb'); } catch (e) { console.warn('MLB Odds API fallback failed:', e && e.message); }
    }
    if (!games || !games.length) {
      console.log('MLB: falling back to ESPN scoreboard');
      try { games = await fetchGamesFromEspn('mlb'); } catch (e) { console.warn('MLB ESPN fallback failed:', e && e.message); }
    }
    return res.json(games || []);
  } catch (err) {
    return sendSportsDataIoError(res, err);
...
app.get('/api/games/soccer', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    let games = [];
    try { games = await buildSoccerGamesResponse(); } catch (e) { console.warn('Soccer primary source failed:', e && e.message); }
    if (!games || !games.length) {
      console.log('Soccer: falling back to The Odds API');
      try { games = await fetchGamesFromOddsApi('soccer'); } catch (e) { console.warn('Soccer Odds API fallback failed:', e && e.message); }
    }
    if (!games || !games.length) {
      console.log('Soccer: falling back to ESPN scoreboard');
      try { games = await fetchGamesFromEspn('soccer'); } catch (e) { console.warn('Soccer ESPN fallback failed:', e && e.message); }
    }
    return res.json(games || []);
  } catch (err) {
    return sendSportsDataIoError(res, err);
...
// --------------------------------------------------------------------------
// v2 Modular Games Routes (/v2/games, /v2/games/:sport, /v2/games/:sport/:league)
// --------------------------------------------------------------------------
const { createRouter: createGamesRouter } = require('./server/routes/games');
app.use('/v2/games', createGamesRouter(authMiddleware, ensureProfileComplete));
...
// --------------------------------------------------------------------------
// Direct ESPN-based logo fallback (no API key needed)
// --------------------------------------------------------------------------
const espnTeamLogoCache = new Map(); // "prefix:normalizedname" → logoUrl
const espnCacheTimestamps = {};       // sportKey → timestamp (ms)
const ESPN_LOGO_CACHE_TTL = 24 * 60 * 60 * 1000; // 24 hours

const ESPN_LOGO_ENDPOINTS = {
  nhl: 'hockey/nhl',
  nfl: 'football/nfl',
  mlb: 'baseball/mlb',
...
async function ensureEspnLogos(sportKey) {
  const now = Date.now();
  if (espnCacheTimestamps[sportKey] && (now - espnCacheTimestamps[sportKey] < ESPN_LOGO_CACHE_TTL)) return;
  try {
    const path = ESPN_LOGO_ENDPOINTS[sportKey];
    if (!path) return;
    const resp = await axios.get(`https://site.api.espn.com/apis/site/v2/sports/${path}/teams`, { timeout: 10000 });
    const leagues = (resp.data && resp.data.sports && resp.data.sports[0] && resp.data.sports[0].leagues) || [];
    const teamEntries = (leagues[0] && leagues[0].teams) || [];
    for (const entry of teamEntries) {
      const t = entry.team || entry;
      const logo = (t.logos && t.logos[0] && t.logos[0].href) || '';
      if (!logo) continue;
      const names = [t.displayName, t.name, t.shortDisplayName, t.nickname, t.abbreviation].filter(Boolean);
      for (const n of names) {
        const normKey = normalizeTeamKey(n);
        if (normKey) {
          espnTeamLogoCache.set(sportKey + ':' + normKey, logo);
          // Also store with generic "soccer" prefix for cross-league lookup
          if (sportKey.startsWith('soccer_')) {
            const soccerKey = 'soccer:' + normKey;
            if (!espnTeamLogoCache.has(soccerKey)) espnTeamLogoCache.set(soccerKey, logo);
          }
        }
      }
    }
    espnCacheTimestamps[sportKey] = now;
    console.log('ESPN logos loaded for', sportKey, '- teams cached:', teamEntries.length);
  } catch (err) {
    console.warn('ESPN logo load failed for ' + sportKey + ':', err && err.message);
  }
}

function espnLogoLookup(prefix, teamName) {
  const norm = normalizeTeamKey(teamName);
  if (!norm) return null;
  // Direct match
  const direct = espnTeamLogoCache.get(prefix + ':' + norm);
  if (direct) return direct;
  // Partial / fuzzy match
  for (const [k, v] of espnTeamLogoCache) {
    if (!k.startsWith(prefix + ':')) continue;
    const kName = k.slice(prefix.length + 1);
    if (kName.includes(norm) || norm.includes(kName)) return v;
  }
  return null;
}

async function getDirectFallbackLogo(sport, name) {
  if (sport === 'nba') return getNbaFallbackLogo(name);
  if (sport === 'nhl') {
    await ensureEspnLogos('nhl');
    return espnLogoLookup('nhl', name);
  }
  if (sport === 'nfl') {
    await ensureEspnLogos('nfl');
    return espnLogoLookup('nfl', name);
  }
  if (sport === 'mlb') {
    await ensureEspnLogos('mlb');
    return espnLogoLookup('mlb', name);
  }
  if (sport === 'soccer') {
    const soccerKeys = Object.keys(ESPN_LOGO_ENDPOINTS).filter(function(k) { return k.startsWith('soccer_'); });
    await Promise.all(soccerKeys.map(function(k) { return ensureEspnLogos(k); }));
    return espnLogoLookup('soccer', name);
  }
  return null;
}
...
app.get('/api/teams/logo', async (req, res) => {
  try {
    ...
    if (!SPORTSDATAIO_KEY && !APISPORTS_KEY) {
      const directLogo = await getDirectFallbackLogo(sport, name);
      if (directLogo) return res.json({ logo: directLogo, team: { name } });
    ...
    if ((!result || !result.logo) && APISPORTS_KEY) {
      try {
        ...
        console.warn('API-Sports team logo lookup failed:', err && err.message);
      }
    }
...
app.get('/api/teams/logo/image', async (req, res) => {
  try {
    ...
    if (!SPORTSDATAIO_KEY && !APISPORTS_KEY) {
      ...
      if (directLogo) {
        try {
          const imgResp = await axios.get(directLogo, { responseType: 'arraybuffer', timeout: 12000 });
          const ct = (imgResp && imgResp.headers && imgResp.headers['content-type']) || 'image/png';
          res.setHeader('Content-Type', ct);
          res.setHeader('Cache-Control', 'public, max-age=86400');
          return res.send(Buffer.from(imgResp.data));
        } catch (pErr) {
          res.setHeader('Cache-Control', 'public, max-age=3600');
          return res.redirect(directLogo);
        }
      }
      res.setHeader('Cache-Control', 'public, max-age=3600');
    ...
    if ((!result || !result.logo) && APISPORTS_KEY) {
      try {
        ...
        console.warn('API-Sports team logo image lookup failed:', err && err.message);
      }
    }
    if (!result || !result.logo) {
      const directLogo = await getDirectFallbackLogo(sport, name);
...
app.post('/api/bets/place', authMiddleware, ensureProfileComplete, async (req, res) => {
  ...
  if (stakeCurrency === 'cash' || stakeCurrency === 'tokens') {
    currency = stakeCurrency;
  }
  if (currency !== 'cash' && currency !== 'tokens') {
    const cashOverride = Number(stakeCash);
    currency = Number.isFinite(cashOverride) && cashOverride > 0 ? 'cash' : 'tokens';
  }
  let stakeValue = Number(stake);
  if (currency === 'cash' && Number.isFinite(Number(stakeCash))) {
    stakeValue = Number(stakeCash);
  }
  if (currency === 'tokens' && Number.isFinite(Number(stakeTokens))) {
    stakeValue = Number(stakeTokens);
  }
  if (!Array.isArray(selections) || !Number.isFinite(stakeValue) || stakeValue <= 0) {
    return res.status(400).json({ error: 'Invalid payload' });
  }
  if (currency === 'tokens' && !Number.isInteger(stakeValue)) {
    return res.status(400).json({ error: 'Token stake must be an integer' });
  }
  // Reject bets on games that have already started
  const nowMs = Date.now();
  if (Array.isArray(selections)) {
    for (const sel of selections) {
      const ct = sel && (sel.commenceTime || sel.startTime || sel.commence_time);
      if (ct) {
        const gameMs = new Date(ct).getTime();
        if (Number.isFinite(gameMs) && gameMs <= nowMs) {
          return res.status(400).json({ error: 'One or more games have already started. Remove them and try again.' });
        }
      }
    }
  }
  try {
    ...
    const result = await firestoreBets.placeBet(uid, {
      type,
      stake: stakeValue,
      selections,
      currency,
      parlayIntent: Boolean(parlayIntent),
      parlayLegs: Number(parlayLegs || 0)
    });
    return res.json({ ok: true, bet: result.bet, rewards: result.rewards, balances: result.balances });
  } catch (err) {
...
app.get('/api/bets', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    ...
    if (String(req.query.settle || '') === '1') {
      try {
        const needsUpdate = (bets || []).filter((bet) => {
          if (!bet || !Array.isArray(bet.selections) || !bet.selections.length) return false;
          if (String(bet.status || '').toLowerCase() === 'pending') return true;
          return bet.selections.some((sel) => !sel || !sel.status);
        });
        if (needsUpdate.length) {
          await resolveBets(needsUpdate, { allowSettle: true });
        }
      } catch (e) {
        console.warn('Settle on read failed:', e && e.message);
      }
    }
