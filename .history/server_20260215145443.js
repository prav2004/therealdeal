...
app.post('/api/wallet/withdraw-request', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    ...
    if (!emailRaw || !/^\S+@\S+\.\S+$/.test(emailRaw)) return res.status(400).json({ error: 'Invalid email' });
...
app.post('/api/support/ticket', async (req, res) => {
  try {
    ...
    if (!subject || subject.length < 3) return res.status(400).json({ error: 'Invalid subject' });
    if (!message || message.length < 10) return res.status(400).json({ error: 'Invalid message' });
...
app.post('/api/careers/apply', (req, res) => {
  upload.single('resume')(req, res, async (err) => {
    ...
    try {
      ...
      if (!about || about.length < 20) return res.status(400).json({ error: 'Tell us more about yourself' });
