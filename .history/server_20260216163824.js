...
function buildOddsMap(oddsList) {
  ...
  (oddsList || []).forEach((game) => {
    ...
    const totalPoints = readStat(game, ['OverUnder', 'Total', 'TotalPoints', 'OverUnderPoints', 'OverUnderTotal', 'TotalScore']);
    const overPayout = readStat(game, ['OverPayout', 'OverMoneyLine', 'OverOdds', 'OverPrice']);
    const underPayout = readStat(game, ['UnderPayout', 'UnderMoneyLine', 'UnderOdds', 'UnderPrice']);
