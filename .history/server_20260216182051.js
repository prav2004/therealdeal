...
async function fetchNbaScheduleWithFallback() {
  ...
  const seasons = [primary, primary - 1].filter((v, i, a) => a.indexOf(v) === i);
  let lastError = null;
  for (const season of seasons) {
    try {
      const payload = await fetchSportsData('nba', `/scores/json/Schedule/${season}`, null, 6 * 60 * 60 * 1000);
      const schedule = Array.isArray(payload) ? payload : [];
      if (schedule.length) return { season: String(season), schedule, error: null };
    } catch (err) {
      lastError = err;
    }
  }
  return { season: String(primary), schedule: [], error: lastError };
}

async function buildNbaNextGamesDebug() {
  const { season, schedule, error } = await fetchNbaScheduleWithFallback();
  if (!schedule.length) {
    ...
    return { season, scheduleCount: 0, nextGamesDate: null, oddsCount: 0, sampleGame: null, sampleOdds: null, error: { status, message } };
  }
