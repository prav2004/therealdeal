...
async function buildNbaNextGamesDebug() {
  const { season, schedule, error, seasonTried } = await fetchNbaScheduleWithFallback();
  if (!schedule.length) {
    ...
    return { season, seasonTried, scheduleCount: 0, nextGamesDate: null, oddsCount: 0, sampleGame: null, sampleOdds: null, error: { status, message } };
  }
  ...
  if (!nextGamesDate) {
    return { season, scheduleCount: schedule.length, nextGamesDate: null, oddsCount: 0, sampleGame: null, sampleOdds: null };
  }
  ...
  const sampleGame = (gamesByDate.get(nextGamesDate) || [])[0] || null;
