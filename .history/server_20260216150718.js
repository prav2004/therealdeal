...
const SPORTS_DATA_IO_KEY = process.env.SPORTS_DATA_IO_KEY || '';
...
async function buildGamesResponse(sport, options = {}) {
  const { games, oddsMap, now, windowMs } = await loadSportsDataGames(sport);
...
app.get('/api/games/nba', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    const games = await buildGamesResponse('nba');
...
app.get('/api/games/soccer', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    const games = await buildGamesResponse('soccer', {
      allowedLeagues: SOCCER_LEAGUE_RULES.map(rule => rule.label),
      leagueResolver: resolveSoccerLeague
    });
    return res.json(games);
  } catch (err) {
    return sendSportsDataIoError(res, err);
