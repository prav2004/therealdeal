...
async function buildNbaNextGamesResponse() {
  const season = String(getNbaSeasonYear());
  const schedulePayload = await fetchSportsData('nba', `/scores/json/Schedule/${season}`, null, 6 * 60 * 60 * 1000);
  const schedule = Array.isArray(schedulePayload) ? schedulePayload : [];
...
async function buildNbaNextGamesDebug() {
  const season = String(getNbaSeasonYear());
  const schedulePayload = await fetchSportsData('nba', `/scores/json/Schedule/${season}`, null, 10 * 60 * 1000);
  const schedule = Array.isArray(schedulePayload) ? schedulePayload : [];
  if (!schedule.length) {
    return { season, scheduleCount: 0, nextGamesDate: null, oddsCount: 0, sampleGame: null, sampleOdds: null };
  }
