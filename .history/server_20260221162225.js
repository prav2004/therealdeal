...
function pickApiSportsMoneyline(response, homeName, awayName) {
      ...
      bets.forEach((bet) => {
        ...
        values.forEach((value) => {
          ...
          if (label.includes('draw') || label === 'x') {
            if (drawMin === null || odd < drawMin) drawMin = odd;
            if (isFanDuel) {
          ...
          if (label.includes('home') || label === '1' || (homeKey && label.includes(homeKey))) {
            if (homeMin === null || odd < homeMin) homeMin = odd;
            if (isFanDuel) {
          ...
          if (label.includes('away') || label === '2' || (awayKey && label.includes(awayKey))) {
            if (awayMin === null || odd < awayMin) awayMin = odd;
            if (isFanDuel) {
  ...
  return {
    home: homeFd || homeDk || homeMin || home,
    away: awayFd || awayDk || awayMin || away,
    draw: drawFd || drawDk || drawMin || draw
  };
}
...
async function buildNbaNextGamesFromApiSports() {
  ...
  const MIN_GAMES_TARGET = 3;
  let nextGamesDate = null;
  let games = [];
  const nowMs = Date.now();
  const NBA_GAME_DURATION_MS = 4 * 60 * 60 * 1000;
  for (let offset = 0; offset <= lookaheadDays; offset += 1) {
    const dateMs = nowMs + (offset * 24 * 60 * 60 * 1000);
    const dateKey = getUtcDateKey(new Date(dateMs));
    const hoursUntil = (Date.UTC(new Date(dateMs).getUTCFullYear(), new Date(dateMs).getUTCMonth(), new Date(dateMs).getUTCDate()) - nowMs) / (60 * 60 * 1000);
    const ttlMs = hoursUntil > 24 ? 6 * 60 * 60 * 1000 : SPORTSDATA_CACHE_TTL.scores;
    const list = await fetchApiSportsGamesByDate(dateKey, season, ttlMs);
    if (list.length) {
      const gamesForDate = list.filter((game) => {
  ...
  if (!games.length || !nextGamesDate) return [];

  if (games.length < MIN_GAMES_TARGET) {
    const nextDateMs = Date.parse(`${nextGamesDate}T00:00:00Z`) + (24 * 60 * 60 * 1000);
    if (Number.isFinite(nextDateMs)) {
      const nextDateKey = getUtcDateKey(new Date(nextDateMs));
      try {
        const nextList = await fetchApiSportsGamesByDate(nextDateKey, season, SPORTSDATA_CACHE_TTL.scores);
        if (nextList.length) {
          const nextUpcoming = nextList.filter((game) => {
            const startRaw = game && game.date && game.date.start
              ? game.date.start
              : (game && game.date ? game.date : null);
            const startMs = startRaw ? Date.parse(startRaw) : NaN;
            if (!Number.isFinite(startMs)) return false;
            return startMs >= nowMs || (startMs + NBA_GAME_DURATION_MS) >= nowMs;
          });
          games = games.concat(nextUpcoming);
        }
      } catch (err) {
  ...
  const nextDayStartMs = Date.parse(`${nextGamesDate}T00:00:00Z`);
  const hoursUntilGames = Number.isFinite(nextDayStartMs)
    ? (nextDayStartMs - nowMs) / (60 * 60 * 1000)
    : 0;
  const oddsTtlMs = hoursUntilGames > 24 ? 6 * 60 * 60 * 1000 : SPORTSDATA_CACHE_TTL.odds;
