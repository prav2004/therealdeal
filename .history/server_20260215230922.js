...
const STORAGE_BUCKET = process.env.FIREBASE_STORAGE_BUCKET || process.env.GCLOUD_STORAGE_BUCKET || 'pickr-d4d9b.firebasestorage.app';
...
async function sendSupportEmail(payload) {
  ...
  if (!/^[A-Z0-9_]+$/.test(screenName)) return res.status(400).json({ error: 'screenName must be A-Z, 0-9, or _ (no spaces)' });
  try {
    await smtpMailer.sendMail({
      from: SMTP_FROM,
      to: SUPPORT_EMAIL_TO,
...
app.get('/api/leaderboard/players', async (req, res) => {
  try {
    ...
    const snap = await firestore.collection('users').orderBy('points', 'desc').limit(limit).get();
    const rows = [];
    snap.forEach((doc) => {
      const u = doc.data() || {};
      const email = String(u.email || '');
      const name = u.fullName || (email ? email.split('@')[0] : 'Player');
      rows.push({
        uid: u.uid || doc.id,
        name,
...
app.get('/api/teams/logo', async (req, res) => {
  try {
    ...
    if (!APISPORTS_KEY) return res.status(503).json({ error: 'API key not configured' });

    const result = await lookupTeamLogo(name, league, season);
...
app.post('/api/profile/photo', authMiddleware, upload.single('photo'), async (req, res) => {
  try {
    ...
    const objectPath = `profile-photos/${uid}.${ext}`;
    ...
    const profilePhotoUrl = `https://firebasestorage.googleapis.com/v0/b/${bucket.name}/o/${encodeURIComponent(objectPath)}?alt=media&token=${token}`;
    await firestoreBets.updateUserProfile(uid, {
      profilePhotoUrl,
    ...
    console.error('Failed to upload profile photo:', err && err.message);
