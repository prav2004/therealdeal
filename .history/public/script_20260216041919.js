(function () {
  ...
  function applyTeamLogo(imgEl, teamName, sportKey) {
    ...
    const proxyUrl = `${apiBase}/api/teams/logo/image?name=${encodeURIComponent(teamName)}&league=12&season=2024${sportQuery}`;
    imgEl.src = fallback;
  ...
  async function loadPicks(sport = 'all', containerOverride = null) {
    ...
    try {
      ...
      if (sport && sport !== 'all' && SPORTSDATA_SPORTS.has(String(sport).toLowerCase())) {
        ...
        const sportsDataUrl = `${API_SPORTSDATA_ODDS()}?sport=${encodeURIComponent(sport)}&date=${encodeURIComponent(dateKey)}&${cacheBuster}`;
        const sportsDataRes = await fetch(sportsDataUrl, { headers, cache: 'no-store' });
        if (sportsDataRes.status === 401) {
          container.innerHTML = '<div class="text-red-600 p-4">Please sign in again to load picks.</div>';
          return;
        }
        if (sportsDataRes.ok) {
          const oddsPayload = await sportsDataRes.json();
          const oddsList = Array.isArray(oddsPayload) ? oddsPayload : [];
          picks = oddsList.map((game) => buildSportsDataEvent(game, String(sport).toLowerCase())).filter(Boolean);
