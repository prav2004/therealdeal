(function () {
  ...
  const PICKS_CACHE_TTL = 60 * 1000;
  const MAX_PICKS_PER_SPORT = 35;
  const MAX_PICKS_ALL = 35;
  let picksCache = { data: null, ts: 0 };

  function filterPicksByWindow(picks, windowMs) {
    const now = Date.now();
    return (picks || [])
      .map(p => ({ pick: p, ts: toMillis(p.commence_time || p.startTime || p.date || null) }))
      .filter(p => p.ts && p.ts >= now && p.ts <= now + windowMs)
      .sort((a, b) => a.ts - b.ts)
      .map(p => p.pick);
  }
  ...
  function filterPicksByWindowWithSport(picks, defaultWindowMs, sportWindows) {
    ...
    return (picks || [])
      .map(p => {
        const ts = toMillis(p.commence_time || p.startTime || p.date || null);
        const sportKey = String(p.sport_key || p.sportKey || '').toLowerCase();
        let windowMs = defaultWindowMs;
        if (sportWindows) {
          Object.entries(sportWindows).forEach(([key, value]) => {
            if (sportKey.includes(String(key).toLowerCase()) && value) windowMs = value;
          });
        }
        return { pick: p, ts, windowMs };
      })
      .filter(p => p.ts && p.ts >= now && p.ts <= now + p.windowMs)
      .sort((a, b) => a.ts - b.ts)
      .map(p => p.pick);
  }
  ...
  function normalizeGameToPick(game) {
    ...
    const displayOdds = {
      home: normalizeMoneyline(game.odds && game.odds.moneyline && game.odds.moneyline.home),
      away: normalizeMoneyline(game.odds && game.odds.moneyline && game.odds.moneyline.away)
    };
    const drawOdd = normalizeMoneyline(game.odds && game.odds.moneyline && game.odds.moneyline.draw);
    if (drawOdd) displayOdds.draw = drawOdd;
    if (!displayOdds.home && !displayOdds.away && !displayOdds.draw) return null;
    return {
  ...
  async function getAllPicksData() {
    ...
    const windowed = filterPicksByWindowWithSport(raw, 24 * 60 * 60 * 1000, {
      nba: 7 * 24 * 60 * 60 * 1000,
      nfl: 14 * 24 * 60 * 60 * 1000,
      nhl: 14 * 24 * 60 * 60 * 1000,
      mlb: 14 * 24 * 60 * 60 * 1000,
      soccer: 14 * 24 * 60 * 60 * 1000
    });
    const picksBySport = {};
    SPORT_TAB_KEYS.forEach((sport) => {
      const list = windowed.filter(p => matchSportKey(p, sport));
  ...
  function createPickCard(pick) {
    ...
    const oddsText = pick.pickTeamValue || (pick.displayOdds && (pick.displayOdds.home || pick.displayOdds.away || pick.displayOdds.draw)) || '';
    ...
    if (pick.displayOdds && pick.displayOdds.draw) btns.appendChild(makeBtn('Draw', pick.displayOdds.draw, true));
