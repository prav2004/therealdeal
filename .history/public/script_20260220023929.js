(function () {
  ...
  function ensureAvatarStyles() {
    ...
    style.textContent = `
      .pickr-avatar{--size:40px;--skin:#f8d7c1;--hair:#1f2937;--lip:#c2410c;--jersey:#0ea5e9;--accent:#0f172a;width:var(--size);height:var(--size);border-radius:999px;background:var(--skin);position:relative;overflow:hidden;border:1px solid rgba(255,255,255,0.18);box-shadow:0 8px 16px rgba(0,0,0,0.25);transform:translateZ(0);} 
  ...
  function createPickCard(pick) {
    ...
    // selection buttons
    const allowDraw = !!(pick.displayOdds && pick.displayOdds.draw && String(sportKey || '').toLowerCase().includes('soccer'));
    ...
    if (allowDraw) btns.appendChild(makeBtn('Draw', pick.displayOdds.draw, true));
    ...
    if (allowDraw && pick.displayOdds && pick.displayOdds.draw) options.push({ team: 'Draw', odds: parseFloat(pick.displayOdds.draw) });
    ...
    try {
      ...
      recent.forEach(({ item, placedAt }) => {
        ...
        selectionItems.forEach((selection, index) => {
          ...
          const legStatus = rawStatus === 'won' || rawStatus === 'win'
            ? 'won'
          ...
          textWrap.appendChild(statusPill);
          if (isParlay) {
