  ...
  function filterPicksByWindow(picks, windowMs, minTs = null, allowMissingTs = false) {
    ...
    return (picks || [])
      .map(p => {
        ...
        if (!Number.isFinite(ts) && allowMissingTs) ts = floorTs;
        return { pick: p, ts };
      })
      .filter(p => Number.isFinite(p.ts) && p.ts >= floorTs && p.ts <= now + windowMs)
      .sort((a, b) => a.ts - b.ts)
      .map(p => p.pick);
  }
  ...
  function filterPicksByWindowWithSport(picks, defaultWindowMs, sportWindows) {
    ...
    return (picks || [])
      .map(p => {
        ...
        if (!Number.isFinite(ts) && allowMissingTs) ts = minTs;
        return { pick: p, ts, windowMs, minTs };
      })
      .filter(p => Number.isFinite(p.ts) && p.ts >= p.minTs && p.ts <= now + p.windowMs)
      .sort((a, b) => a.ts - b.ts)
      .map(p => p.pick);
  }
