(function () {
  ...
  async function loadRecentBets(options = {}) {
    const { containerId = 'betHistoryContainer', hours = 72, showAll = false, attempt = 0 } = options;
    ...
    if (summaryRange) summaryRange.textContent = showAll ? 'All time' : `${hours}h rolling`;

    const cutoff = showAll ? 0 : Date.now() - hours * 60 * 60 * 1000;
    ...
    try {
      ...
      let recent = bets
        ...
        .filter((b) => showAll || b.placedAt >= cutoff)
        .sort((a, b) => b.placedAt - a.placedAt);

      if (recent.length === 0) {
        const localHistory = readJSON('betHistory', []);
        if (localHistory.length > 0) {
          recent = localHistory
            .map((item) => ({
              item,
              placedAt: toMillis(item.date || item.placedAt || 0) || Date.now()
            }))
            .filter((b) => showAll || b.placedAt >= cutoff)
            .sort((a, b) => b.placedAt - a.placedAt);
        }
      ...
      if (recent.length === 0) {
        ...
        if (totalBets && attempt < 1) {
          setTimeout(() => {
            loadRecentBets({ containerId, hours, showAll, attempt: attempt + 1 });
          }, 1200);
          container.innerHTML = '<div class="empty-state">Syncing your bets. One moment...</div>';
          return;
        }
        const emptyMessage = showAll
          ? 'No bets yet. Try the Sports tab to place your first one.'
          : 'No bets in the last 72 hours. Try the Sports tab to place a new one.';
        container.innerHTML = `
        ...
        if (refreshBtn) {
          refreshBtn.addEventListener('click', () => {
            loadRecentBets({ containerId, hours, showAll, attempt: 0 });
          });
        }
        return;
      }
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (recentBetsEl) {
      setInterval(() => { loadRecentBets({ containerId: 'betHistoryContainer', hours: 72, showAll: false, attempt: 0 }); }, 60 * 60 * 1000);
    }
