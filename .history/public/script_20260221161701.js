(function () {
  ...
  function filterPicksByWindowWithSport(picks, defaultWindowMs, sportWindows) {
    ...
    return (picks || [])
      ...
      .filter(p => p.ts && p.ts >= p.minTs && p.ts <= now + p.windowMs)
      .sort((a, b) => a.ts - b.ts)
      .map(p => p.pick);
  }
  ...
  async function getAllPicksData() {
    ...
    SPORT_TAB_KEYS.forEach((sport) => {
      ...
      const list = filterPicksByWindow(baseList, windowMs, minTs);
