(function () {
  ...
  async function getAllPicksData() {
    ...
    const results = await Promise.all(SPORT_TAB_KEYS.map((sport) => fetchSport(sport)));
    ...
    const windowed = filterPicksByWindow(raw, 24 * 60 * 60 * 1000);
    const picksBySport = {};
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (sportTabsContainer) {
      ...
      (async () => {
        const { data, firstAvailable } = await refreshTabs();
        if (!firstAvailable) {
          const container = getPicksContainer();
          if (container) container.innerHTML = '<div class="text-gray-400 p-6">No bets available atm.</div>';
          return;
        }
