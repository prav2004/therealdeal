(function () {
  ...
  function ensureAvatarStyles() {
    ...
    style.textContent = `
      .pickr-avatar{--size:40px;--skin:#f8d7c1;--hair:#1f2937;--lip:#c2410c;--jersey:#0ea5e9;--accent:#0f172a;width:var(--size);height:var(--size);border-radius:999px;background:var(--skin);position:relative;overflow:hidden;border:1px solid rgba(255,255,255,0.18);box-shadow:0 8px 16px rgba(0,0,0,0.25);} 
      .pickr-avatar .hair{position:absolute;left:-6%;top:-10%;width:112%;height:48%;background:var(--hair);border-bottom-left-radius:60% 80%;border-bottom-right-radius:60% 80%;}
      ...
      .pickr-avatar.hair-spike .hair{height:46%;clip-path:polygon(0 100%,12% 40%,24% 100%,38% 35%,50% 100%,62% 40%,76% 100%,90% 38%,100% 100%);} 
  ...
  function getAvatarPreset(avatarId) {
    return AVATAR_PRESETS[avatarId] || AVATAR_PRESETS['avatar-1'];
  }
  ...
  async function loadPicks(sport = 'all', containerOverride = null) {
    ...
    try {
      ...
      let picks = [];
      let loadedFromSportsData = false;

      if (sport && sport !== 'all' && SPORTSDATA_SPORTS.has(String(sport).toLowerCase())) {
        try {
          const { list } = await fetchSportsDataOddsWithLookahead(String(sport).toLowerCase(), headers, 7);
          picks = list.map((game) => buildSportsDataEvent(game, String(sport).toLowerCase())).filter(Boolean);
          loadedFromSportsData = picks.length > 0;
        } catch (err) {
      ...
      if (!loadedFromSportsData) {
        const picksUrl = API_PICKS() + (qs ? `${qs}&${cacheBuster}` : `?${cacheBuster}`);
        const res = await fetch(picksUrl, { headers, cache: 'no-store' });
        if (res.status === 401) {
          container.innerHTML = '<div class="text-red-600 p-4">Please sign in again to load picks.</div>';
          return;
        }
        const data = await res.json();
        picks = Array.isArray(data) ? data.slice(0, 50) : (Array.isArray(data.picks) ? data.picks.slice(0, 50) : []);
      }
      const now = Date.now();
      ...
      const withTimes = picks
        ...
        .filter(p => p.ts && p.ts >= now && p.ts <= windowEnd)
        .sort((a, b) => a.ts - b.ts);
      const maxGames = 10;
      picks = withTimes.slice(0, maxGames).map(p => p.pick);
      if (sport === 'nba') {
  ...
  async function loadNbaStatsSection() {
    ...
    const optionsEl = document.getElementById('nbaStatOptions');
    const searchInput = document.getElementById('nbaPlayerSearch');
    const searchBtn = document.getElementById('nbaPlayerSearchBtn');
    const statsEl = document.getElementById('nbaStatsPanel');
    const statusEl = document.getElementById('nbaStatsStatus');
    const hintEl = document.getElementById('nbaSearchHint');
    if (!optionsEl || !searchInput || !searchBtn || !statsEl) return;
    ...
    const fetchJson = async (path) => {
      ...
      if (!res.ok) {
        ...
        try { error.payload = await res.json(); } catch (e) { /* ignore */ }
        throw error;
      }
      return res.json();
    };
    ...
    const readStatValue = (payload, keys) => {
      if (!payload || !keys || !keys.length) return null;
      for (const key of keys) {
        if (payload && typeof payload[key] !== 'undefined' && payload[key] !== null) return payload[key];
      }
      return null;
    };
    const formatStatValue = (value, format) => {
      ...
      if (Math.abs(num) >= 100 || Number.isInteger(num)) return String(num);
      return num.toFixed(1);
    };

    const NBA_STAT_OPTIONS = [
      { key: 'pts', label: 'Points', keys: ['PointsPerGame', 'Points'] },
      { key: 'reb', label: 'Rebounds', keys: ['ReboundsPerGame', 'Rebounds'] },
      { key: 'ast', label: 'Assists', keys: ['AssistsPerGame', 'Assists'] },
      { key: 'stl', label: 'Steals', keys: ['StealsPerGame', 'Steals'] },
      { key: 'blk', label: 'Blocks', keys: ['BlockedShotsPerGame', 'BlockedShots'] },
      { key: 'tov', label: 'Turnovers', keys: ['TurnoversPerGame', 'Turnovers'] },
      { key: 'min', label: 'Minutes', keys: ['MinutesPerGame', 'Minutes'] },
      { key: 'fg', label: 'FG%', keys: ['FieldGoalsPercentage'], format: 'pct' },
      { key: 'fg3', label: '3P%', keys: ['ThreePointersPercentage'], format: 'pct' }
    ];
    ...
    const renderOptions = () => {
      optionsEl.innerHTML = '';
      NBA_STAT_OPTIONS.forEach((option) => {
        const btn = document.createElement('button');
        btn.type = 'button';
        btn.className = 'tab-chip px-3 py-2 rounded-full text-xs font-semibold';
        if (selectedStat && selectedStat.key === option.key) btn.classList.add('active');
        btn.textContent = option.label;
        btn.addEventListener('click', () => {
          selectedStat = option;
          optionsEl.querySelectorAll('.active').forEach((el) => el.classList.remove('active'));
          btn.classList.add('active');
          statsEl.textContent = 'Search a player to view the selected stat.';
          setHint(`Selected stat: ${option.label}. Now search a player.`);
        });
    ...
    const buildPlayerIndex = (players) => {
      return players.map((player) => {
        const fullName = player.FullName
          || player.Name
          || (player.FirstName && player.LastName ? `${player.FirstName} ${player.LastName}` : (player.FirstName || player.LastName || 'Player'));
        return {
          id: player.PlayerID || player.PlayerId || player.ID || player.id || null,
          name: fullName || 'Player',
          team: player.Team || player.TeamName || player.TeamKey || '',
          position: player.Position || player.Pos || player.PositionCategory || '',
          norm: normalizeName(fullName)
        };
      }).filter((player) => player.id && player.norm);
    };

    const loadPlayers = async () => {
      try {
        setStatus('Loading NBA players…');
        const playersPayload = await fetchJson('/api/sportsdata/players?sport=nba');
        const players = Array.isArray(playersPayload) ? playersPayload : [];
        playerIndex = buildPlayerIndex(players);
        if (!playerIndex.length) {
          setStatus('No NBA players available.');
          setHint('Try again later.');
          return false;
        }
        setStatus(`Loaded ${playerIndex.length} players.`);
        return true;
      } catch (err) {
        const status = err && err.status ? String(err.status) : '';
        const detail = err && err.payload && err.payload.error ? String(err.payload.error) : '';
        if (err && err.status === 401) {
          setStatus('Sign in again to load NBA players.');
          setHint('Your session may have expired.');
        } else if (err && err.status === 403) {
          setStatus('Complete onboarding to view NBA players.');
          setHint('Finish onboarding, then return here.');
        } else if (err && err.status === 503) {
          setStatus('NBA data temporarily unavailable.');
          setHint('Service is warming up. Try again soon.');
        } else {
          const extra = detail ? ` (${detail})` : '';
          const msg = err && err.message ? ` ${String(err.message)}` : '';
          setStatus(`Failed to load NBA players${status ? ` [${status}]` : ''}${extra}.${msg}`);
    ...
    const findPlayer = (query) => {
      ...
      if (!norm) return null;
      const exact = playerIndex.find((player) => player.norm === norm);
      if (exact) return exact;
      const contains = playerIndex.find((player) => player.norm.includes(norm) || norm.includes(player.norm));
      return contains || null;
    };

    const renderResult = (player, option, payload) => {
      const season = payload && (payload.Season || payload.SeasonYear || payload.SeasonID || payload.SeasonType || payload.SeasonName);
      const statValue = readStatValue(payload, option.keys);
      const displayValue = formatStatValue(statValue, option.format);
      const seasonLabel = season ? `Season ${season}` : 'Current season';
      statsEl.innerHTML = `
        <div class="text-base font-semibold mb-2">${escapeHtml(player.name)}</div>
        <div class="text-xs text-slate-400 mb-4">${escapeHtml(player.team || 'NBA')} • ${escapeHtml(player.position || '—')}</div>
        <div class="rounded-lg bg-slate-900/70 p-4">
          <div class="text-xs uppercase tracking-widest text-slate-400 mb-2">${escapeHtml(option.label)}</div>
          <div class="text-2xl font-semibold text-slate-100">${escapeHtml(displayValue)}</div>
          <div class="text-[10px] uppercase tracking-widest text-slate-500 mt-2">${escapeHtml(seasonLabel)}</div>
        </div>
      `;
    };
    ...
    const runSearch = async () => {
      ...
      if (!selectedStat) {
        setHint('Select a stat first.');
        statsEl.textContent = 'Select a stat and search a player.';
        return;
      }
      if (!query) {
        setHint('Enter a player name.');
        return;
      }
      if (!playerIndex.length) {
      ...
      if (!player) {
        statsEl.textContent = 'No player found.';
        setHint('Try the full first and last name.');
        return;
      }
      statsEl.textContent = 'Loading stats…';
      setStatus(`Loading ${selectedStat.label} for ${player.name}…`);
      try {
        const payload = await fetchJson(`/api/sportsdata/playerstats?sport=nba&playerId=${encodeURIComponent(player.id)}`);
        const statsPayload = Array.isArray(payload) ? payload[0] : payload;
        renderResult(player, selectedStat, statsPayload || {});
        setHint(`Showing ${selectedStat.label} for ${player.name}.`);
      } catch (err) {
        statsEl.textContent = 'Stats unavailable.';
        setStatus('Failed to load player stats.');
      }
    };

    renderOptions();
    statsEl.textContent = 'Select a stat and search a player.';
    setHint('Select a stat, then search a player.');
    await loadPlayers();
  ...
  window.PickrApp = {
    loadPicks, loadWalletPage, loadRecentBets, refreshHeaders, refreshAvatars, addSelection
  };
