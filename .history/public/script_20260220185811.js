(function () {
  ...
  function createPickCard(pick) {
    ...
    const oddsCandidates = [oddsHome, oddsAway, oddsDraw].filter((v) => Number.isFinite(v));
    if (!oddsCandidates.length) {
      console.warn('Skipping pick with no valid odds', { matchId, sportKey, marketType });
      return null;
    }
    const bestOdds = oddsCandidates.length ? Math.max(...oddsCandidates) : null;
    const oddsLabel = bestOdds ? `${Number(bestOdds).toFixed(2)}x` : '—';
    ...
    if (!btns.children.length) return null;
