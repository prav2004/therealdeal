(function () {
  ...
  async function getAllPicksData() {
    ...
    const res = await fetch(API_PICKS() + `?${cacheBuster}`, { headers, cache: 'no-store' });
    if (res.status === 401) throw new Error('auth');
    const data = await res.json();
    const raw = Array.isArray(data) ? data : (Array.isArray(data.picks) ? data.picks : []);
    ...
    SPORT_TAB_KEYS.forEach((sport) => {
      ...
      picksBySport[sport] = list.slice(0, 12);
    });
    const all = windowed.slice(0, 24);
