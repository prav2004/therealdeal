  ...
  async function loadPicks(sport = 'all', containerOverride = null) {
    ...
    try {
      ...
      if (sport && sport !== 'all' && SPORTSDATA_SPORTS.has(String(sport).toLowerCase())) {
        ...
        const sportsDataUrl = `${API_SPORTSDATA_ODDS()}?sport=${encodeURIComponent(sport)}&date=${encodeURIComponent(dateKey)}`;
        const sportsDataRes = await fetch(sportsDataUrl, { headers });
