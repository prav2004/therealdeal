  ...
  function setupConfirmBet() {
    ...
    confirmBtn.addEventListener('click', async () => {
      ...
      const successFlow = async () => {
        ...
        await wait(BET_RESULT_HOLD_MS);
        hideBetOverlay();
        showBetToast('success', 'Bet Successfully Placed ✔');
      };
  ...
  function filterPicksByWindowWithSport(picks, defaultWindowMs, sportWindows) {
    ...
    return (picks || [])
      .map(p => {
        ...
        if (sportKey.includes('nba') && sportWindows && sportWindows.nba) windowMs = sportWindows.nba;
    ...
    SPORT_TAB_KEYS.forEach((sport) => {
      ...
      const windowMs = sport === 'nba'
        ...
        : sport === 'nhl'
          ? 14 * 24 * 60 * 60 * 1000
          : 24 * 60 * 60 * 1000;
      const list = filterPicksByWindow(baseList, windowMs);
