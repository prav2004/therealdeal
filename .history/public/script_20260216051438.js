(function () {
  ...
  async function loadNbaStatsSection() {
    ...
    const renderPlayerBio = (player) => {
      const name = player.fullName || player.name || 'Player';
      const position = player.position || player.Position || '—';
      const height = player.height || player.Height || '—';
      const weight = player.weight || player.Weight || '—';
      const team = player.teamName || player.Team || player.team || '—';
      statsEl.innerHTML = `
        <div class="text-base font-semibold mb-2">${escapeHtml(String(name))}</div>
        <div class="text-xs text-slate-400 mb-4">Bio (stats unavailable)</div>
        <div class="grid grid-cols-2 gap-3 text-xs">
          <div class="rounded-lg bg-slate-900/70 p-3">
            <div class="text-slate-400 uppercase tracking-widest text-[10px]">Team</div>
            <div class="mt-2 text-slate-100">${escapeHtml(String(team))}</div>
          </div>
          <div class="rounded-lg bg-slate-900/70 p-3">
            <div class="text-slate-400 uppercase tracking-widest text-[10px]">Position</div>
            <div class="mt-2 text-slate-100">${escapeHtml(String(position))}</div>
          </div>
          <div class="rounded-lg bg-slate-900/70 p-3">
            <div class="text-slate-400 uppercase tracking-widest text-[10px]">Height</div>
            <div class="mt-2 text-slate-100">${escapeHtml(String(height))}</div>
          </div>
          <div class="rounded-lg bg-slate-900/70 p-3">
            <div class="text-slate-400 uppercase tracking-widest text-[10px]">Weight</div>
            <div class="mt-2 text-slate-100">${escapeHtml(String(weight))}</div>
          </div>
        </div>
      `;
    };
    ...
    if (useSportsDataFallback || !games.length) {
      try {
        ...
        const oddsResult = await fetchSportsDataOddsWithLookahead('nba', headers, 2);
        const events = (oddsResult.list || []).map((game) => buildSportsDataEvent(game, 'nba')).filter(Boolean);
        const teamMap = new Map();
        events.forEach((ev) => {
          const home = ev.home_team || '';
          const away = ev.away_team || '';
          if (home) teamMap.set(normalizeTeamKey(home), { teamId: normalizeTeamKey(home), name: home, teamKey: normalizeTeamKey(home) });
          if (away) teamMap.set(normalizeTeamKey(away), { teamId: normalizeTeamKey(away), name: away, teamKey: normalizeTeamKey(away) });
        });
        fallbackTeams = Array.from(teamMap.values());
        const playersPayload = await fetchJson('/api/sportsdata/players?sport=nba');
        fallbackPlayers = Array.isArray(playersPayload) ? playersPayload : [];
        games = [];
        useSportsDataFallback = true;
      } catch (err) {
    ...
    teams.forEach((team) => {
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          let players = [];
          if (useSportsDataFallback) {
            const teamKey = normalizeTeamKey(team.teamKey || team.name || team.fullName || team.teamId || '');
            players = fallbackPlayers.filter((player) => {
              const pTeam = normalizeTeamKey(player.Team || player.TeamName || player.TeamKey || player.TeamID || player.TeamID || '');
              return pTeam && teamKey && (pTeam === teamKey || pTeam.includes(teamKey) || teamKey.includes(pTeam));
            }).map((player) => ({
              playerId: player.PlayerID || player.PlayerId || player.ID || null,
              fullName: player.Name || player.FullName || player.FirstName && player.LastName ? `${player.FirstName} ${player.LastName}` : player.Name || 'Player',
              position: player.Position || player.Pos || player.PositionCategory || '',
              teamName: player.Team || player.TeamName || player.TeamKey || team.name || '',
