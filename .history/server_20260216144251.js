...
async function fetchLatestPicks() {
  ...
  if (apiKey) {
    try {
      ...
      console.warn('Warning: failed to fetch events from Odds API, retrying h2h only. Error:', err.message);
      try {
        ...
        events = Array.isArray(fallback.data) ? fallback.data : [];
      } catch (fallbackErr) {
        console.warn('Warning: failed to fetch h2h events from Odds API. Error:', fallbackErr.message);
        events = [];
      }
    }
  }
  if (!events.length && SPORTSDATAIO_KEY) {
    try {
      ...
      if (sportsDataEvents.length) events = sportsDataEvents;
    } catch (err) {
