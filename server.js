...
async function fetchNbaService(servicePath, params = {}) {
...
// Serve the login page explicitly for the `/login` path. Express does not
// automatically map `/login` to `login.html` in the public folder, so we
// define this route to send the HTML file. This ensures that navigating to
// `/login` loads the correct login page instead of returning a 404 error.
...
// Redirect the root path to the login page by default. When unauthenticated
// users visit `/` or `/index.html`, they should be sent to the login page.
// Later, this logic can be enhanced to check authentication status and
// redirect authenticated users to the main app instead.
app.get('/', (req, res) => {
  res.redirect('/login');
});
...
async function ensureProfileComplete(req, res, next) {
  try {
    ...
    if (!profile.profileComplete) return res.status(403).json({ error: 'Profile incomplete: complete onboarding at /profile' });
    return next();
  } catch (err) {
...
// ─── Mount modular games router (/api/games) ─────────────────────────────────
const { createRouter: createGamesRouter } = require('./server/routes/games');
app.use('/api/games', createGamesRouter(authMiddleware, ensureProfileComplete));
 ...
 */
function generateFillerPicks(count) {
  /**
  ...
  const sportKeys = Object.keys(sportsTeams);
  const picks = [];
  for (let i = 0; i < count; i++) {
    // Choose whether to generate a standard matchup or a player prop
    const makePlayerProp = Math.random() < 0.3; // 30% chance for a player prop
    const sportKey = sportKeys[Math.floor(Math.random() * sportKeys.length)];
    const teams = sportsTeams[sportKey];
    const now = Date.now();
    const startTime = new Date(now + Math.random() * 3 * 24 * 60 * 60 * 1000).toISOString(); // within 3 days
    const matchId = `gen-${Date.now()}-${i}-${Math.random().toString(36).substring(2, 6)}`;

    if (makePlayerProp) {
      // Simple synthetic player prop
      const players = ['LeBron James','Stephen Curry','Kevin Durant','Patrick Mahomes','Aaron Judge','Connor McDavid','Erling Haaland','Kylian Mbappé','Luka Dončić','Giannis Antetokounmpo'];
      const player = players[Math.floor(Math.random() * players.length)];
      const multiplier = Number((1.4 + Math.random() * 1.2).toFixed(2));
      const id = `${matchId}_prop_${player.replace(/\s+/g, '')}`;
      picks.push({
        id,
        match_id: matchId,
        // For player props, put the player in home_team for display purposes
        home_team: player,
        away_team: 'Player Prop',
        commence_time: startTime,
        sport_key: sportKey,
        // Provide displayOdds so the frontend can render a single option
        displayOdds: { home: multiplier },
        // Keep legacy fields some client code might read
        team: player,
        multiplier
      });
    } else {
      // Standard team matchup
      ...
      // Generate odds: ensure both sides have a decimal > 1.2
...
async function fetchLatestPicks() {
  ...
  let existing = (await readPicks()).filter((event) => {
    try {
      return new Date(event.commence_time).getTime() > nowMs;
    } catch (e) {
      return false;
    }
  });

  // Attempt to fetch new events from the Odds API if a key is provided. If
  ...
  if (apiKey) {
    try {
      const response = await axios.get(
        `https://api.the-odds-api.com/v4/sports/upcoming/odds`,
        {
          params: {
            ...
            regions: 'us',
            markets: 'h2h',
            // Request decimal odds so that the frontend can display multiplier‑style odds
            oddsFormat: 'decimal',
            dateFormat: 'iso'
          }
        }
      ...
      console.warn('Warning: failed to fetch events from Odds API, will generate synthetic picks. Error:', err.message);
      events = [];
    }
  }
  // Merge the newly fetched events into the existing list, avoiding duplicates.
  for (const event of events) {
    // Skip if the event is already in our list
  ...
  const now = new Date();
  existing = existing.filter((event) => {
    try {
      const start = new Date(event.commence_time || event.start_time);
      return start > now; // keep any future events
    } catch (e) {
      return false;
    }
  });
  ...
  // development and manual QA where the odds API may be empty or limited.
  const MIN_PICKS = 10;
  ...
  // Only keep the first 50 picks. If there are fewer than 50, keep them all (no filler picks)
  if (existing.length > 50) {
    existing = existing.slice(0, 50);
  }

  // Compute the best available decimal odds for each outcome (home, away, draw) across
  // all bookmakers. This property is attached to the event object so the
  // frontend can quickly display multiplier‑style odds. When the API returns
  // decimal odds, the price is already in decimal form; otherwise, we convert
  // from American odds. Draw/Tie is only applicable for soccer events.
  const toDecimal = (price, oddsFormat) => {
    // If odds are already decimal we simply return them.
    if (oddsFormat === 'decimal' || typeof price === 'number') {
      return price;
    }
    // Convert American odds to decimal. Negative price: (100 - price) / -price, positive price: (price + 100) / 100.
    if (price < 0) return (100 - price) / -price;
    return (price + 100) / 100;
  };

  existing = existing.map(event => {
    // Determine if API provided decimal or American odds. When markets list
    // odds in decimal the type is number; for American odds it is integer.
    // We'll infer the format by inspecting the first outcome's price.
    ...
    if (event.bookmakers && event.bookmakers.length) {
      ...
      if (market && market.outcomes.length && typeof market.outcomes[0].price === 'number') {
        // Price is a number and likely already decimal
        oddsFormat = 'decimal';
      }
    }
    let bestHome = null;
    ...
    // Attach displayOdds for frontend use. Round to 2 decimal places.
    event.displayOdds = {};
...
// Map short sport keys (from SportsData.io / frontend) to Odds API keys
const SHORT_TO_ODDS_API = {
  nba: 'basketball_nba',
...
function normalizeToOddsApiKey(sportKey) {
  if (!sportKey) return sportKey;
  const k = sportKey.toLowerCase().trim();
  // Already in full format?
  if (k.includes('_')) return k;
  return SHORT_TO_ODDS_API[k] || k;
}
...
async function settlePendingBets() {
  ...
  try {
    ...
    const now = Date.now();
    const STALE_MS = 10 * 24 * 60 * 60 * 1000; // 10 days

    // ── Phase 0: Auto-void very old pending bets ──
    const staleBets = [];
    const activeBets = [];
    for (const bet of pendingBets) {
      const placedMs = bet.placedAt
        ? (typeof bet.placedAt.toMillis === 'function' ? bet.placedAt.toMillis()
          : typeof bet.placedAt.seconds === 'number' ? bet.placedAt.seconds * 1000
          : 0)
        : 0;
      if (placedMs > 0 && (now - placedMs) > STALE_MS) {
        staleBets.push(bet);
      } else {
        activeBets.push(bet);
      }
    ...
    for (const bet of staleBets) {
      try {
        await firestoreBets.settleBet(bet.betId || bet.id, { status: 'void' });
        console.log('Auto-voided stale bet:', bet.betId || bet.id);
      } catch (e) {
        console.warn('Failed to void stale bet', bet.betId || bet.id, e && e.message);
      }
    }

    if (activeBets.length === 0) return;

    // ── Phase 1: Try Odds API settlement ──
    ...
    if (missingEventIds.size) {
      try {
        ...
        console.warn('Failed to look up missing eventIds:', e && e.message);
      }
    }

    const sportToEventIds = new Map();
    // Normalize short sport keys (nba -> basketball_nba) for Odds API lookups
    eventIdToSport.forEach((sportKey, eventId) => {
      if (!sportKey) return;
      const normalized = normalizeToOddsApiKey(sportKey);
      const set = sportToEventIds.get(normalized) || new Set();
      set.add(eventId);
      sportToEventIds.set(normalized, set);
    });

    const resultsByEvent = new Map();
    if (apiKey) {
      // Fetch all sports in parallel for speed
      const sportEntries = Array.from(sportToEventIds.entries());
      console.log('Settlement: fetching Odds API scores for', sportEntries.map(([k, v]) => `${k}(${v.size} events)`).join(', '));
      const scoreResults = await Promise.allSettled(
        sportEntries.map(([sportKey]) => fetchScoresForSport(sportKey, apiKey))
      );
      scoreResults.forEach((result, idx) => {
        if (result.status === 'fulfilled') {
          const completedCount = (result.value || []).filter(ev => ev && ev.completed).length;
          console.log(`  ${sportEntries[idx][0]}: ${(result.value || []).length} events (${completedCount} completed)`);
          (result.value || []).forEach(ev => {
            if (ev && ev.id) resultsByEvent.set(ev.id, ev);
          });
        } else {
          console.warn(`  ${sportEntries[idx][0]}: FAILED -`, result.reason && result.reason.message);
        }
      });
    }

    let settledCount = 0;
    const stillPending = []; // bets not resolved by Odds API
    console.log(`Settlement: ${activeBets.length} active bets, ${resultsByEvent.size} events with results`);
    for (const bet of activeBets) {
      ...
      const selectionOutcomes = selections.map((sel, i) => ({
        eventId: sel.eventId,
        outcome: outcomes[i]
      }));
      try {
        await firestoreBets.settleBet(bet.betId || bet.id, { status, selectionOutcomes });
        settledCount += 1;
      } catch (e) {
    ...
    // ── Phase 2: ESPN fallback for bets not resolved by Odds API ──
    if (stillPending.length > 0) {
      settledCount += await settleWithEspnFallback(stillPending);
    }

    if (settledCount) console.log('Settled bets:', settledCount);
    if (staleBets.length) console.log('Auto-voided stale bets:', staleBets.length);
  } catch (err) {
...
// --------------------------------------------------------------------------
// ESPN fallback settlement — uses free ESPN scoreboard to settle bets
// when The Odds API doesn't have results.
// --------------------------------------------------------------------------
const ODDS_TO_ESPN_MAP = {
  // Full Odds API keys
  basketball_nba: { slug: 'basketball', league: 'nba' },
  americanfootball_nfl: { slug: 'football', league: 'nfl' },
  icehockey_nhl: { slug: 'hockey', league: 'nhl' },
  baseball_mlb: { slug: 'baseball', league: 'mlb' },
  basketball_ncaab: { slug: 'basketball', league: 'mens-college-basketball' },
  americanfootball_ncaaf: { slug: 'football', league: 'college-football' },
  ...
  // Short keys (from SportsData.io / frontend)
  nba: { slug: 'basketball', league: 'nba' },
  nfl: { slug: 'football', league: 'nfl' },
  nhl: { slug: 'hockey', league: 'nhl' },
  mlb: { slug: 'baseball', league: 'mlb' },
  ncaab: { slug: 'basketball', league: 'mens-college-basketball' },
  ncaaf: { slug: 'football', league: 'college-football' },
  epl: { slug: 'soccer', league: 'eng.1' },
  mls: { slug: 'soccer', league: 'usa.1' },
};

function fuzzyTeamMatch(oddsTeam, espnTeam) {
  if (!oddsTeam || !espnTeam) return false;
  const a = normalizeName(oddsTeam).replace(/[^a-z0-9 ]/g, '');
  const b = normalizeName(espnTeam).replace(/[^a-z0-9 ]/g, '');
  if (a === b) return true;
  // Check if one contains the other (handles "LA Lakers" vs "Los Angeles Lakers")
  if (a.includes(b) || b.includes(a)) return true;
  // Check last-word match (city-less comparison: "Lakers" vs "Lakers")
...
async function fetchEspnScoreboard(slug, league) {
  try {
    // Fetch today + past 3 days to catch recently completed games
    const allEvents = [];
    const seen = new Set();
      ...
      dateFetches.push(
        axios.get(url, { timeout: 10000 })
          .then(resp => {
            ...
            console.log(`ESPN fetch ${slug}/${league} date=${dateStr}: ${events.length} events`);
            return events;
          })
          .catch(err => {
            console.warn(`ESPN fetch FAILED ${slug}/${league} date=${dateStr}: ${err && err.message}`);
            return [];
          })
      );
    }
    ...
    console.warn('ESPN scoreboard fetch failed:', slug, league, e && e.message);
    return [];
  }
}

function determineOutcomeFromEspn(selection, espnGame) {
  if (!selection || !espnGame) return 'pending';
  const comp = espnGame.competitions && espnGame.competitions[0];
  if (!comp) return 'pending';
  const statusType = (comp.status && comp.status.type) || {};
  if (statusType.state !== 'post') return 'pending'; // game not completed
  ...
  if (fuzzyTeamMatch(selection.pick, winnerName)) return 'win';
  // Check if pick matches the losing team
  const loserName = homeScore > awayScore ? awayName : homeName;
  if (fuzzyTeamMatch(selection.pick, loserName)) return 'lose';
  return 'pending'; // couldn't match
}

function findEspnMatch(selection, espnEvents) {
  ...
  for (const ev of espnEvents) {
    ...
    if (fuzzyTeamMatch(selHome, homeName) && fuzzyTeamMatch(selAway, awayName)) return ev;
    if (fuzzyTeamMatch(selHome, awayName) && fuzzyTeamMatch(selAway, homeName)) return ev;
    // Fallback: match if pick team appears in either competitor
    if (fuzzyTeamMatch(selection.pick, homeName) || fuzzyTeamMatch(selection.pick, awayName)) {
      if (selHome && (fuzzyTeamMatch(selHome, homeName) || fuzzyTeamMatch(selHome, awayName))) return ev;
    }
  }
  return null;
}

async function settleWithEspnFallback(pendingBets) {
  console.log(`ESPN fallback: processing ${pendingBets.length} pending bets`);
  // Collect unique sport keys (both raw and normalized)
  const sportKeys = new Set();
  for (const bet of pendingBets) {
    for (const sel of (bet.selections || [])) {
      const sk = sel.sportKey || sel.sport_key || '';
      if (sk) {
        sportKeys.add(sk);
        // Also add normalized version in case the raw key doesn't match
        const normalized = normalizeToOddsApiKey(sk);
        if (normalized !== sk) sportKeys.add(normalized);
      }
    }
  }

  // Fetch ESPN scoreboards for all relevant sports
  const espnEventsBySport = new Map();
  const fetchPromises = [];
  const fetched = new Set(); // deduplicate same slug/league
  for (const sk of sportKeys) {
    const mapping = ODDS_TO_ESPN_MAP[sk];
    if (!mapping) continue;
    const cacheKey = `${mapping.slug}_${mapping.league}`;
    if (fetched.has(cacheKey)) {
      // Reuse already-queued results under this key too
      continue;
    }
    fetched.add(cacheKey);
    fetchPromises.push(
      fetchEspnScoreboard(mapping.slug, mapping.league).then(events => {
        // Store events under all keys that map to the same slug/league
        for (const k of sportKeys) {
          const m = ODDS_TO_ESPN_MAP[k];
          if (m && `${m.slug}_${m.league}` === cacheKey) {
            espnEventsBySport.set(k, events);
          }
        }
      })
    );
  ...
  // Log ESPN fetch results
  for (const [sk, events] of espnEventsBySport.entries()) {
    const completed = events.filter(ev => {
      const comp = ev.competitions && ev.competitions[0];
      const st = comp && comp.status && comp.status.type;
      return st && st.state === 'post';
    }).length;
    console.log(`ESPN fallback: ${sk} → ${events.length} events (${completed} completed)`);
  }
  ...
  for (const bet of pendingBets) {
    ...
    const outcomes = selections.map(sel => {
      const sk = sel.sportKey || sel.sport_key || '';
      const events = espnEventsBySport.get(sk) || [];
      const match = findEspnMatch(sel, events);
      const outcome = !match ? 'pending' : determineOutcomeFromEspn(sel, match);
      if (!match) {
        console.log(`  ESPN: no match for "${sel.pick}" (${sel.homeTeam || sel.home_team || '?'} vs ${sel.awayTeam || sel.away_team || '?'}) sportKey=${sk} events=${events.length}`);
      } else {
        console.log(`  ESPN: matched "${sel.pick}" → ${outcome}`);
      }
      return outcome;
    });
    let status = null;
    ...
    try {
      await firestoreBets.settleBet(bet.betId || bet.id, { status, selectionOutcomes });
      settledCount += 1;
      console.log('ESPN fallback settled bet:', bet.betId || bet.id, status);
    } catch (e) {
      console.warn('ESPN fallback settle failed', bet.betId || bet.id, e && e.message);
    }
  }
  return settledCount;
}
...
// --------------------------------------------------------------------------
// Leaderboard endpoint (Firestore-based, no external API)
// --------------------------------------------------------------------------
...
// --------------------------------------------------------------------------
// Team logo endpoints (uses ESPN free CDN — no API key required)
// --------------------------------------------------------------------------
const ESPN_SPORT_MAP = {
  ...
  nhl: { slug: 'hockey', league: 'nhl' },
  mlb: { slug: 'baseball', league: 'mlb' },
  soccer: { slug: 'soccer', league: 'eng.1' },
  epl: { slug: 'soccer', league: 'eng.1' },
  laliga: { slug: 'soccer', league: 'esp.1' },
  bundesliga: { slug: 'soccer', league: 'ger.1' },
  seriea: { slug: 'soccer', league: 'ita.1' },
  ucl: { slug: 'soccer', league: 'uefa.champions' },
  fifa: { slug: 'soccer', league: 'fifa.world' }
};

const _espnTeamCache = {};
async function findEspnLogo(teamName, sport) {
  const key = (sport || 'nba').toLowerCase().replace(/[^a-z]/g, '');
  const mapping = ESPN_SPORT_MAP[key] || ESPN_SPORT_MAP.nba;
  const cacheKey = `${mapping.slug}_${mapping.league}`;

  if (!_espnTeamCache[cacheKey]) {
    try {
      const url = `https://site.api.espn.com/apis/site/v2/sports/${mapping.slug}/${mapping.league}/teams?limit=200`;
      const resp = await axios.get(url, { timeout: 8000 });
      const teams = (resp.data && resp.data.sports && resp.data.sports[0] &&
        resp.data.sports[0].leagues && resp.data.sports[0].leagues[0] &&
        resp.data.sports[0].leagues[0].teams) || [];
      _espnTeamCache[cacheKey] = teams.map(t => t.team || t);
    } catch (e) {
      console.warn('ESPN teams fetch failed for', cacheKey, e && e.message);
      _espnTeamCache[cacheKey] = [];
    }
  }

  const teams = _espnTeamCache[cacheKey];
  const needle = teamName.toLowerCase().replace(/[^a-z0-9]/g, '');

  // Exact match first, then substring
  let match = teams.find(t => {
    const full = (t.displayName || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    const short = (t.shortDisplayName || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    const abbr = (t.abbreviation || '').toLowerCase();
    const nick = (t.nickname || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    const loc = (t.location || '').toLowerCase().replace(/[^a-z0-9]/g, '');
    return full === needle || short === needle || abbr === needle || nick === needle || loc === needle;
  });
  if (!match) {
    match = teams.find(t => {
      const full = (t.displayName || '').toLowerCase().replace(/[^a-z0-9]/g, '');
      ...
      return full.includes(needle) || needle.includes(full) ||
             short.includes(needle) || needle.includes(short) ||
             nick.includes(needle) || needle.includes(nick) ||
             loc.includes(needle) || needle.includes(loc);
    });
  }

  if (match && match.logos && match.logos.length > 0) {
    return { logo: match.logos[0].href, team: { name: match.displayName || teamName } };
  }

  // If no league matched, try all leagues for soccer
  if (key === 'soccer' || !match) {
    for (const [k, m] of Object.entries(ESPN_SPORT_MAP)) {
      if (k === key) continue;
      const ck = `${m.slug}_${m.league}`;
      if (!_espnTeamCache[ck]) {
        try {
          const url = `https://site.api.espn.com/apis/site/v2/sports/${m.slug}/${m.league}/teams?limit=200`;
          const resp = await axios.get(url, { timeout: 8000 });
          const ts = (resp.data && resp.data.sports && resp.data.sports[0] &&
            resp.data.sports[0].leagues && resp.data.sports[0].leagues[0] &&
            resp.data.sports[0].leagues[0].teams) || [];
          _espnTeamCache[ck] = ts.map(t => t.team || t);
        } catch (e) {
          _espnTeamCache[ck] = [];
        }
      }
      const ts2 = _espnTeamCache[ck];
      const m2 = ts2.find(t => {
        const full = (t.displayName || '').toLowerCase().replace(/[^a-z0-9]/g, '');
        const short = (t.shortDisplayName || '').toLowerCase().replace(/[^a-z0-9]/g, '');
        return full.includes(needle) || needle.includes(full) || short.includes(needle) || needle.includes(short);
      });
      if (m2 && m2.logos && m2.logos.length > 0) {
        return { logo: m2.logos[0].href, team: { name: m2.displayName || teamName } };
      }
    }
  }
...
function getFallbackAvatarLogo(name) {
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=1a1a2e&color=00d4ff&size=128&bold=true`;
}
...
app.get('/api/teams/logo', async (req, res) => {
  try {
    ...
    const result = await findEspnLogo(name, sport);
...
// --------------------------------------------------------------------------
// Live Scores endpoint (uses ESPN free scoreboard API — no key required)
// --------------------------------------------------------------------------
const _scoreboardCache = {};

// All soccer leagues to fetch when sport=soccer
const SOCCER_LEAGUES = [
  { key: 'epl', slug: 'soccer', league: 'eng.1', label: 'Premier League' },
  { key: 'laliga', slug: 'soccer', league: 'esp.1', label: 'La Liga' },
  { key: 'bundesliga', slug: 'soccer', league: 'ger.1', label: 'Bundesliga' },
  { key: 'seriea', slug: 'soccer', league: 'ita.1', label: 'Serie A' },
  { key: 'ligue1', slug: 'soccer', league: 'fra.1', label: 'Ligue 1' },
  { key: 'ucl', slug: 'soccer', league: 'uefa.champions', label: 'Champions League' },
  { key: 'mls', slug: 'soccer', league: 'usa.1', label: 'MLS' },
  { key: 'ligamx', slug: 'soccer', league: 'mex.1', label: 'Liga MX' }
];

function parseScoreboardEvents(events, leagueLabel) {
  return (events || []).map(ev => {
    ...
    // Extract lightweight MLB situation (batter, on-deck, in-hole) for live score cards
    const situation = comp.situation || null;
    let mlbSituation = null;
    if (situation) {
      const extractPlayerLite = (p) => {
        if (!p) return null;
        const ath = p.athlete || {};
        return {
          displayName: p.displayName || ath.displayName || '',
          summary: p.summary || '',
      ...
      mlbSituation = {
        batter: extractPlayerLite(situation.batter),
        onDeck: extractPlayerLite(situation.onDeck),
        inHole: extractPlayerLite(situation.inHole),
        outs: situation.outs != null ? situation.outs : null,
    ...
    return {
      ...
      homeTeam: {
        ...
        logo: (home.team && home.team.logo) || (home.team && home.team.logos && home.team.logos[0] && home.team.logos[0].href) || '',
        score: home.score || '0',
      ...
      awayTeam: {
        ...
        logo: (away.team && away.team.logo) || (away.team && away.team.logos && away.team.logos[0] && away.team.logos[0].href) || '',
        score: away.score || '0',
...
function parseDetailedScoreboardEvents(events, leagueLabel) {
  return (events || []).map(ev => {
    const comp = ev.competitions && ev.competitions[0];
    if (!comp) return null;
    ...
    const extractLinescores = (competitor) => {
      return (competitor.linescores || []).map((ls, i) => ({
        period: i + 1,
        value: ls.value != null ? String(ls.value) : '0'
      }));
    };

    // Helper to extract player info (batter, pitcher, on-deck, in-hole)
    ...
    // Extract leaders from event level or competitor level
    const extractLeaders = () => {
      const leaders = [];
      // Try event-level leaders first (most common)
      const evLeaders = ev.leaders || [];
      evLeaders.forEach(cat => {
        const top = cat.leaders && cat.leaders[0];
        if (top) {
          const ath = top.athlete || {};
          leaders.push({
            category: cat.displayName || cat.name || '',
            shortName: cat.shortDisplayName || cat.abbreviation || '',
            displayValue: top.displayValue || '',
            athlete: {
              displayName: ath.displayName || ath.shortName || '',
              headshot: ath.headshot || '',
              jersey: ath.jersey || '',
      ...
      // Fallback: try competitor-level leaders
      if (leaders.length === 0) {
    ...
    // Extract per-team leaders (for team toggle view)
    const extractTeamLeaders = (team) => {
      const leaders = [];
      (team.leaders || []).forEach(cat => {
    ...
    // Extract probable starting pitcher for a competitor
    const extractProbable = (team) => {
      const probables = team.probables || [];
      if (probables.length === 0) return null;
      const p = probables[0];
      const ath = p.athlete || {};
      const stats = {};
      (p.statistics || []).forEach(s => { stats[s.name || s.abbreviation] = s.displayValue || ''; });
      return {
        ...
        wins: stats.wins || stats.W || null,
        losses: stats.losses || stats.L || null,
        saves: stats.saves || stats.SV || null
      };
    };

    // Extract team-level statistics (H, R, AVG, ERA, etc. with ranks)
    ...
    const broadcasts = (comp.broadcasts || []).map(b => {
      const names = (b.names || []);
      return { type: (b.type && b.type.shortName) || '', names };
    });
    ...
    return {
      ...
      homeTeam: {
        ...
        hits: home.hits != null ? String(home.hits) : null,
        errors: home.errors != null ? String(home.errors) : null,
        leaders: extractTeamLeaders(home),
        probable: extractProbable(home),
        teamStats: extractTeamStats(home)
      },
      awayTeam: {
        name: (away.team && away.team.displayName) || '',
        abbreviation: (away.team && away.team.abbreviation) || '',
        ...
        hits: away.hits != null ? String(away.hits) : null,
        errors: away.errors != null ? String(away.errors) : null,
        leaders: extractTeamLeaders(away),
        probable: extractProbable(away),
        teamStats: extractTeamStats(away)
      },
      situation: situation ? {
        batter: extractPlayer(situation.batter),
        pitcher: extractPlayer(situation.pitcher),
        onDeck: extractPlayer(situation.onDeck),
        inHole: extractPlayer(situation.inHole),
        onFirst: !!situation.onFirst,
        onSecond: !!situation.onSecond,
        onThird: !!situation.onThird,
        balls: situation.balls != null ? situation.balls : null,
        strikes: situation.strikes != null ? situation.strikes : null,
        outs: situation.outs != null ? situation.outs : null,
        ...
        lastPlayType: situation.lastPlay && situation.lastPlay.type ? (situation.lastPlay.type.text || '') : '',
        lastPlayScoreValue: situation.lastPlay ? (situation.lastPlay.scoreValue || 0) : 0,
      ...
      leaders: extractLeaders(),
      odds: odds ? {
        details: odds.details || '',
        overUnder: odds.overUnder || null,
        spread: odds.spread || null,
        homeMoneyLine: (odds.homeTeamOdds && odds.homeTeamOdds.moneyLine) || null,
        awayMoneyLine: (odds.awayTeamOdds && odds.awayTeamOdds.moneyLine) || null
      } : null,
      broadcasts: broadcasts.length > 0 ? broadcasts : null,
      venue: venue ? {
        name: venue.fullName || '',
        city: (venue.address && venue.address.city) || '',
        state: (venue.address && venue.address.state) || ''
      } : null,
      notes: (ev.notes && ev.notes.length > 0) ? ev.notes.map(n => n.headline || n.text || '').filter(Boolean) : null
    };
  }).filter(Boolean);
}
...
app.get('/api/scores/live', async (req, res) => {
  try {
    ...
    // For soccer, fetch all leagues in parallel
    if (sport === 'soccer') {
      const soccerCacheKey = 'soccer_all';
      if (_scoreboardCache[soccerCacheKey] && (now - _scoreboardCache[soccerCacheKey].ts) < 2000) {
        return res.json(_scoreboardCache[soccerCacheKey].data);
      }

      const results = await Promise.allSettled(
        SOCCER_LEAGUES.map(async (lg) => {
          const url = `https://site.api.espn.com/apis/site/v2/sports/${lg.slug}/${lg.league}/scoreboard`;
          const resp = await axios.get(url, { timeout: 6000 });
          return { label: lg.label, events: (resp.data && resp.data.events) || [] };
        })
      );
      ...
      for (const r of results) {
        if (r.status === 'fulfilled') {
          allGames = allGames.concat(parseScoreboardEvents(r.value.events, r.value.label));
        }
      }

      // Deduplicate by game ID (in case leagues overlap, e.g. UCL teams)
      const seen = new Set();
      ...
      const result = { sport, games: allGames, lastUpdated: new Date().toISOString() };
      _scoreboardCache[soccerCacheKey] = { data: result, ts: now };
      return res.json(result);
    }

    // Non-soccer: single league fetch
    const mapping = ESPN_SPORT_MAP[sport] || ESPN_SPORT_MAP.nba;
    const cacheKey = `${mapping.slug}_${mapping.league}`;

    // Cache for 2 seconds for real-time live updates
    if (_scoreboardCache[cacheKey] && (now - _scoreboardCache[cacheKey].ts) < 2000) {
      return res.json(_scoreboardCache[cacheKey].data);
    }

    const url = `https://site.api.espn.com/apis/site/v2/sports/${mapping.slug}/${mapping.league}/scoreboard`;
    const resp = await axios.get(url, { timeout: 6000 });

    const events = (resp.data && resp.data.events) || [];
    const games = parseScoreboardEvents(events, '');

    const result = { sport, games, lastUpdated: new Date().toISOString() };
    _scoreboardCache[cacheKey] = { data: result, ts: now };
    return res.json(result);
  } catch (err) {
    console.error('Live scores fetch failed:', err && err.message);
    return res.status(500).json({ error: 'Failed to fetch live scores' });
  }
});

// --------------------------------------------------------------------------
// MLB Batting Lineup endpoint (full 9-hitter lineup from ESPN summary)
// --------------------------------------------------------------------------
const _lineupCache = {};

app.get('/api/mlb/lineup/:eventId', async (req, res) => {
  try {
    ...
    if (_lineupCache[cacheKey] && (now - _lineupCache[cacheKey].ts) < 15000) {
      return res.json(_lineupCache[cacheKey].data);
    }
    ...
    // Labels from boxscore batting: ['H-AB', 'AB', 'R', 'H', 'RBI', 'HR', 'BB', 'K', '#P', 'AVG', 'OBP', 'SLG']
    const teams = [];

    for (const psec of boxPlayers) {
      const team = psec.team || {};
      const battingSection = (psec.statistics || []).find(s => s.type === 'batting');
      if (!battingSection) continue;

      const labels = battingSection.labels || [];
      const athletes = battingSection.athletes || [];
      const avgIdx = labels.indexOf('AVG');
      const obpIdx = labels.indexOf('OBP');
      const slgIdx = labels.indexOf('SLG');
      ...
      for (const a of athletes) {
        ...
        const headshot = ath.headshot;
        const headshotUrl = typeof headshot === 'object' ? (headshot.href || '') : (headshot || '');

        roster.push({
          batOrder: a.batOrder || 0,
          starter: !!a.starter,
          displayName: ath.displayName || ath.shortName || '',
          shortName: ath.shortName || '',
          headshot: headshotUrl,
          position: pos.abbreviation || '',
          avg: avgIdx >= 0 ? (stats[avgIdx] || '-') : '-',
          obp: obpIdx >= 0 ? (stats[obpIdx] || '-') : '-',
          slg: slgIdx >= 0 ? (stats[slgIdx] || '-') : '-',
          hab: habIdx >= 0 ? (stats[habIdx] || '-') : '-',
          r: rIdx >= 0 ? (stats[rIdx] || '0') : '0',
          h: hIdx >= 0 ? (stats[hIdx] || '0') : '0',
          hr: hrIdx >= 0 ? (stats[hrIdx] || '0') : '0',
          rbi: rbiIdx >= 0 ? (stats[rbiIdx] || '0') : '0',
          bb: bbIdx >= 0 ? (stats[bbIdx] || '0') : '0',
          k: kIdx >= 0 ? (stats[kIdx] || '0') : '0'
        });
      }

      // Sort by bat order
      roster.sort((a, b) => a.batOrder - b.batOrder);

      // Mark top hitters: highest season AVG gets a star
      if (roster.length > 0) {
        const validAvg = roster.filter(p => p.avg !== '-' && parseFloat(p.avg) > 0);
        if (validAvg.length > 0) {
          // Top AVG hitter
          const topAvg = validAvg.reduce((best, p) => parseFloat(p.avg) > parseFloat(best.avg) ? p : best);
          topAvg.starAvg = true;
          // Top HR hitter
          const validHr = roster.filter(p => parseInt(p.hr) > 0 || (p.avg !== '-'));
          if (validHr.length > 0) {
            const topHr = validHr.reduce((best, p) => parseInt(p.hr) > parseInt(best.hr) ? p : best);
            if (parseInt(topHr.hr) > 0) topHr.starHR = true;
          }
          // Top RBI hitter
          const validRbi = roster.filter(p => parseInt(p.rbi) > 0 || (p.avg !== '-'));
          if (validRbi.length > 0) {
            const topRbi = validRbi.reduce((best, p) => parseInt(p.rbi) > parseInt(best.rbi) ? p : best);
            if (parseInt(topRbi.rbi) > 0) topRbi.starRBI = true;
          }
        }
      }

      teams.push({
        homeAway: (psec.displayOrder === 2 || (team.abbreviation && rosters.length > 0 && rosters[0].team && rosters[0].team.abbreviation === team.abbreviation && rosters[0].homeAway === 'home')) ? 'home' : 'away',
    ...
    // Determine home/away correctly from rosters data
    for (const r of rosters) {
      const rTeam = r.team || {};
      const match = teams.find(t => t.abbreviation === rTeam.abbreviation);
      if (match) match.homeAway = r.homeAway || match.homeAway;
    }

    const awayTeam = teams.find(t => t.homeAway === 'away') || teams[1] || null;
    const homeTeam = teams.find(t => t.homeAway === 'home') || teams[0] || null;
    ...
    _lineupCache[cacheKey] = { data: result, ts: now };
    return res.json(result);
  } catch (err) {
    console.error('MLB lineup fetch failed:', err && err.message);
    return res.status(500).json({ error: 'Failed to fetch MLB lineup' });
  }
});

// --------------------------------------------------------------------------
// MLB Live Action endpoint (pitch map + spray chart from ESPN summary)
// --------------------------------------------------------------------------
const _playsCache = {};

app.get('/api/mlb/plays/:eventId', async (req, res) => {
  try {
    const eventId = req.params.eventId;
    if (!eventId || !/^\d+$/.test(eventId)) {
      return res.status(400).json({ error: 'Invalid event ID' });
    ...
    const now = Date.now();
    const cacheKey = `plays_${eventId}`;
    if (_playsCache[cacheKey] && (now - _playsCache[cacheKey].ts) < 10000) {
      return res.json(_playsCache[cacheKey].data);
    }
    ...
    // Build athlete name lookup from rosters + boxscore
    const athleteNames = {};
    for (const r of (summary.rosters || [])) {
      for (const entry of (r.roster || [])) {
        const ath = entry.athlete || entry;
        if (ath.id) athleteNames[ath.id] = ath.shortName || ath.displayName || '';
      }
    }
    for (const psec of ((summary.boxscore && summary.boxscore.players) || [])) {
      for (const statSec of (psec.statistics || [])) {
        for (const a of (statSec.athletes || [])) {
          const ath = a.athlete || {};
          if (ath.id) athleteNames[ath.id] = ath.shortName || ath.displayName || athleteNames[ath.id] || '';
        }
      }
    }

    // Group plays into at-bats preserving order
    const atBatGroups = {};
    const atBatOrder = [];
    for (const play of allPlays) {
      const abId = play.atBatId;
      if (!abId) continue;
      if (!atBatGroups[abId]) {
        atBatGroups[abId] = [];
        atBatOrder.push(abId);
      }
      atBatGroups[abId].push(play);
    }

    const skipTypes = new Set(['Play Result', 'End Batter/Pitcher', 'Start Batter/Pitcher']);

    const atBatList = atBatOrder.map(abId => {
      const plays = atBatGroups[abId];
      const resultPlay = plays.find(p => p.type && p.type.text === 'Play Result');
      const pitchPlays = plays.filter(p => p.pitchCoordinate && !skipTypes.has((p.type && p.type.text) || ''));
      const hitPlay = plays.find(p => p.hitCoordinate && p.type && (p.type.text === 'Play Result' || p.type.text === 'Double' || p.type.text === 'Single' || p.type.text === 'Triple' || p.type.text === 'Home Run' || p.type.text === 'Ball In Play' || p.type.text === 'Foul Ball' || p.type.text.includes('Out')));
      // fallback: any play with hitCoordinate
      const hitFinal = hitPlay || plays.find(p => p.hitCoordinate);

      // Get batter/pitcher from first play's participants
      let batterName = '', pitcherName = '';
      for (const p of (plays[0].participants || [])) {
        const aid = p.athlete && p.athlete.id;
        if (p.type === 'batter' && aid) batterName = athleteNames[aid] || '';
        if (p.type === 'pitcher' && aid) pitcherName = athleteNames[aid] || '';
      }

      return {
        atBatId: abId,
        inning: (plays[0].period && plays[0].period.displayValue) || '',
        batter: batterName,
        pitcher: pitcherName,
        result: resultPlay ? (resultPlay.text || '') : '',
        resultType: resultPlay && resultPlay.type ? resultPlay.type.text : '',
        scoringPlay: plays.some(p => p.scoringPlay),
        pitches: pitchPlays.map(p => ({
          num: p.atBatPitchNumber || 0,
          type: (p.type && p.type.text) || '',
          pitchType: p.pitchType ? (p.pitchType.abbreviation || '') : '',
          pitchName: p.pitchType ? (p.pitchType.text || '') : '',
          velocity: p.pitchVelocity || null,
          x: p.pitchCoordinate.x,
          y: p.pitchCoordinate.y,
          count: p.resultCount || null
        })),
        hit: hitFinal && hitFinal.hitCoordinate ? {
          x: hitFinal.hitCoordinate.x,
          y: hitFinal.hitCoordinate.y,
          trajectory: hitFinal.trajectory || '',
          type: (hitFinal.type && hitFinal.type.text) || ''
        } : null
      };
    });
    ...
    _playsCache[cacheKey] = { data: result, ts: now };
    return res.json(result);
  } catch (err) {
    console.error('MLB plays fetch failed:', err && err.message);
    return res.status(500).json({ error: 'Failed to fetch MLB plays' });
  }
});

// --------------------------------------------------------------------------
// Detailed Live Scores endpoint (includes situation, linescore, leaders, etc.)
// --------------------------------------------------------------------------
const _detailScoreboardCache = {};

app.get('/api/scores/live/detail', async (req, res) => {
  try {
    const sport = String(req.query.sport || 'nba').toLowerCase();
    const gameId = req.query.id || null;
    const now = Date.now();

    // For soccer, fetch all leagues in parallel
    if (sport === 'soccer') {
      const cacheKey = 'detail_soccer_all';
      if (_detailScoreboardCache[cacheKey] && (now - _detailScoreboardCache[cacheKey].ts) < 2000) {
        const cached = _detailScoreboardCache[cacheKey].data;
        if (gameId) {
          const game = cached.games.find(g => g.id === gameId);
          return res.json({ sport, game: game || null, lastUpdated: cached.lastUpdated });
        }
      ...
      for (const r of results) {
        if (r.status === 'fulfilled') {
          allGames = allGames.concat(parseDetailedScoreboardEvents(r.value.events, r.value.label));
        }
      }
      const seen = new Set();
      ...
      const result = { sport, games: allGames, lastUpdated: new Date().toISOString() };
      _detailScoreboardCache[cacheKey] = { data: result, ts: now };
      if (gameId) {
        const game = allGames.find(g => g.id === gameId);
        return res.json({ sport, game: game || null, lastUpdated: result.lastUpdated });
      }
      return res.json(result);
    }
    ...
    // Non-soccer: single league fetch
    const mapping = ESPN_SPORT_MAP[sport] || ESPN_SPORT_MAP.nba;
    const cacheKey = `detail_${mapping.slug}_${mapping.league}`;
    ...
    const resp = await axios.get(url, { timeout: 6000 });
    const events = (resp.data && resp.data.events) || [];
    const games = parseDetailedScoreboardEvents(events, '');
    ...
    console.error('Detailed live scores fetch failed:', err && err.message);
    return res.status(500).json({ error: 'Failed to fetch detailed live scores' });
  }
});
...
// Settle pending bets every 15 minutes using Odds API + ESPN fallback.
cron.schedule('*/15 * * * *', () => {
  settlePendingBets();
});
  ...
  app.get('/api/me', authMiddleware, async (req, res) => {
    try {
      ...
      return res.json({
        ...
        cash: Math.round(Number(profile.cashBalance || 0) * 100) / 100,
        cashBalance: Math.round(Number(profile.cashBalance || 0) * 100) / 100,
        stats: profile.stats || { wins: 0, losses: 0, pending: 0, totalBets: 0 },
        // Task / quest state
...
// Check if a screen name is available (case-insensitive)
app.get('/api/check-screenname', authMiddleware, async (req, res) => {
  try {
...
app.post('/api/profile', authMiddleware, async (req, res) => {
  try {
    ...
    const screenName = (payload.screenName || '').trim();
    const avatarId = (payload.avatarId || '').trim();
...
app.get('/api/picks', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    ...
    if (sport) {
      ...
      filtered = picks.filter(event => (event.sport_key || '').toLowerCase().includes(search));
    }
    // Sort by commence_time to ensure the earliest games appear first
...
app.post('/api/bets/place', authMiddleware, ensureProfileComplete, async (req, res) => {
  const uid = req.uid;
  const { selections, stake, type = 'single', currency, stakeCash, stakeTokens, parlayIntent, parlayLegs } = req.body || {};
  if (!Array.isArray(selections) || !Number.isFinite(stake) || stake <= 0) {
    return res.status(400).json({ error: 'Invalid payload' });
  }
  try {
    // Delegate to Firestore-backed placement which performs transactional balance updates
    const result = await firestoreBets.placeBet(uid, { type, stake, selections, currency, stakeCash, stakeTokens, parlayIntent, parlayLegs });
 ...
 * Pass ?settle=1 to trigger on-demand settlement (throttled to once per 5 min).
 */
let _lastSettleMs = 0;
const SETTLE_COOLDOWN = 60 * 1000; // 1 minute cooldown (was 5 min)

app.get('/api/bets', authMiddleware, ensureProfileComplete, async (req, res) => {
  try {
    const uid = req.uid;

    // Fire settlement in background (non-blocking) so bets return instantly
    if (req.query.settle === '1' && Date.now() - _lastSettleMs > SETTLE_COOLDOWN) {
      _lastSettleMs = Date.now();
      settlePendingBets().catch(e => console.warn('On-demand settle failed:', e && e.message));
    }
